...
export function assembleRookiePanel({
  totalsByYear, ppgByYear, positionOf, birthdateOf, draftInfoOf,
  fromYear, toYear, minOutcomeGames = 6,
}) {
  const rows = [];
  const coverage = { assembled: 0, surviving: 0, drops: {}, hitCapCount: 0 };
  ...
  for (let Y = fromYear; Y <= toYear; Y++) {
    ...
    for (const pid of Object.keys(seasonTotals)) {
      ...
      const proj = reconstructRookieProjection({
        position, ageAtDraft,
        draftRound: draftInfo.draftRound ?? null,
        draftPick: draftInfo.draftPick ?? null,
      });

      const outcomeLookup = computeSeasonPoints(pid, ppgByYear[Y + 1]);
