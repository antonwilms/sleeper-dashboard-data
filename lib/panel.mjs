...
import {
  ...
  reconstructRookieProjection,
  REGRESSION_MODELS, CURRENT_REGRESSION_MODEL,
...
export function computeSeasonPoints(pid, outcomesForYear) {
  if (!outcomesForYear) return { ppg: null, gamesPlayed: 0, totalPts: 0 };
  const rec = outcomesForYear.get(String(pid));
  if (!rec) return { ppg: null, gamesPlayed: 0, totalPts: 0 };
  return { ppg: rec.actualPPG, gamesPlayed: rec.actualGames ?? 0, totalPts: rec.actualTotalPts ?? 0 };
}
...
export function attachFactorMultipliers(rows, ctx) {
  const {
    ...
    // Fix pass 1 item 1 — the panel's own toYear, needed to reconstruct
    // teamOffense under CURRENT-team attribution (see the teamOffense setup
    // below). Optional: a test ctx without it falls back to the latest
    // loaded year.
    toYear = null,
    // D-17 — versioned Step 4 regression bucket. Defaults to the app's current model.
    regressionModel = CURRENT_REGRESSION_MODEL,
    // In-season backtest seams (bin/backtest.mjs --inseason, scripts/inseason-run.mjs).
    // Defaults reproduce every committed artifact byte-for-byte.
    // requirePositiveOutcome=false skips ONLY the nonPositiveOutcome drop (in-season rows
    // carry outcomePPG: null; the outcome is attached later, per checkpoint/horizon).
    // depthOrderOf(pid, position, lastQSeason, lastQTeam) replaces the inner
    // resolveDepthOrder in Step 8 (returns a 1-based order or null); the sentinelHit and
    // coverage bookkeeping runs unchanged on its result.
    requirePositiveOutcome = true, depthOrderOf = null,
  } = ctx;

  if (!REGRESSION_MODELS.includes(regressionModel)) {
    throw new Error(`[panel] unknown regression model '${regressionModel}' — use ${REGRESSION_MODELS.join('|')}`);
  }
  ...
  const fitCoverage = {
    ...
    nonPositiveAnchor: 0,
    byYearCoverage: {}, // D6b — { "POS|factor": { [year]: {total, eligible} } }, all 13 factors
    regressionModel,
  };
  ...
  // Fix pass 1 item 1 — teamOffense (Step 7) is pinned to CURRENT-team
  // attribution, not the per-season-team attribution every other D6a factor
  // correctly uses: computeTeamContext's only caller, App.jsx:214, passes
  // {attribution: 'current-team'}, overriding the module default of
  // per-season-team (finding 7's original text was wrong about this one
  // function — corrected in Step 0/§Design/A in this same change). Reuses
  // this file's own established current-team convention (teamKeyResolver):
  // ignore season, resolve every pid's team at a single anchor year — toYear
  // when the caller supplies it (the real --fit run always does), else the
  // latest loaded year as the best available stand-in. This is deliberately
  // a SEPARATE, differently-attributed team-totals map from teamTotalsByYear
  // (built under whichever attribution the whole panel run is pinned to,
  // e.g. per-season-team for --fit) — teamOffense alone needs its own.
  const teamOffenseAnchorYear = toYear ?? allYears[allYears.length - 1];
  const currentTeamOf = teamKeyResolver('current-team', totalsByYear, teamOffenseAnchorYear);
  const teamOffenseTotalsByYear = {};
  const getTeamOffenseTotals = (Y) => {
    if (!teamOffenseTotalsByYear[Y]) teamOffenseTotalsByYear[Y] = buildTeamTotalsForSeason(totalsByYear[Y], Y, currentTeamOf);
    return teamOffenseTotalsByYear[Y];
  };
  const teamOffenseRanksByYear = {};
  const getTeamOffenseRanks = (Y) => {
    if (!teamOffenseRanksByYear[Y]) teamOffenseRanksByYear[Y] = buildTeamOffenseRanks(getTeamOffenseTotals(Y));
  ...
  // Fix pass 1 item 2 — qbQuality's eligible window is EMPTY, not merely
  // narrow (finding 4's most extreme case): no KTC coverage anywhere in the
  // 2013-2025 panel window, the dynastyScore branch is unportable, and the
  // app's own quality population is fantasy-roster-scoped (unreconstructable
  // offline). `eligibleWindow: []` records that explicitly rather than
  // leaving it to be inferred from `byYear.*.eligible` all reading 0; D6b
  // must read this and exclude qbQuality from the ablation entirely (§E) —
  // a factor held at 1.0 by construction always looks prunable.
  fitCoverage.qbQuality = { notApplicable: 0, noQb1: 0, resolved: 0, byYear: {}, eligibleWindow: [], ungraded: true };
  ...
  for (const row of rows) {
    ...
    if (fullFactors.includes('regression')) {
      ...
      multipliers.regression = reconstructRegressionFactor(ppgs, meanPPG, lastQ.ppg, { position, model: regressionModel });
    }
    ...
    if (d6Factors.includes('depth')) {
      const depthOrder = typeof depthOrderOf === 'function'
        ? depthOrderOf(pid, position, lastQSeason, lastQTeam)
        : resolveDepthOrder(lastQSeason, lastQTeam, position, pid);
    ...
    if (d6Factors.includes('teamOffense')) {
      ...
      const rowTeamY = currentTeamOf(pid, Y) ?? null; // CURRENT-team, not row.team (per-season) — Fix pass 1 item 1
    ...
    if (requirePositiveOutcome && !(row.outcomePPG > 0)) { drop('nonPositiveOutcome'); continue; }
    ...
    // D6b addition: a GENERIC per-(position,factor,year) eligible/total
    // counter for ALL thirteen factors uniformly (fitCoverage.byYearCoverage),
    // so §D/§E can report "the eligible window and coverage rate for every
    // factor" without each factor needing its own bespoke byYear field.
    for (const factor of [...fullFactors, ...d6Factors]) {
      ...
      const key = `${position}|${factor}`;
      const yearBucket = (fitCoverage.byYearCoverage[key] ??= {});
      const yb = (yearBucket[Y] ??= { total: 0, eligible: 0 });
      yb.total++;
      if (!sentinelHit[factor]) yb.eligible++;
    }
    ...
    fitRows.push({
      ...
      // D6b Step 4 injury-gated proxy — dnpWeeks on the SAME season
      // regressionFactor's outlierRatio reads (lastQSeason), since that is
      // the season whose "underperformance" a bounce-back correction would
      // be explaining. See runStep4Verdict's own deviation note (the
      // hand-authored enrichment/injuries.json has zero entries).
      dnpWeeksLastQ: lastQRec?.dnpWeeks ?? 0,
    });
  }
...
// ═══ D6b — the widened, correctly-composed pipeline + calibration/ablation ═══
// (fullpipeline-harness.md §D/§E + the Decision's sensitivity guard). Pure;
// operates on rows already produced by attachFactorMultipliers (multipliers/
// anchorBasePPG/qualifyingSeasons already attached).

// "D6b's first task" (Fix pass 1 item 3 / the Decision): the real, composed
// product across all thirteen factors, with the correct clamp ordering for
// compBlend — a post-outer-clamp multiplicative stage, not a log-additive
// term (see attachFactorMultipliers' own compBlend branch, which this
// generalizes so the ablation/sensitivity check can hold any subset at 1).
// Deliberately a NEW function, not a rewrite of predictWithExponents: that
// function stays scoped to the ORIGINAL seven-factor R3-FIT calibration
// engine (fitExponents/gradeExponentFit), untouched by this slice — D6b
// needs no exponent fit, only a hand-composed (w=1 everywhere) point
// estimate per row, which is all this computes.
//
// holdAtOne: factor names to exclude from the log-sum (== forcing that
// factor's multiplier to 1.0). 'compBlend' in holdAtOne skips the whole
// post-clamp multiplication instead (there is no log term for a post-clamp
// ratio).
export function predictFullPipeline(row, { holdAtOne = [] } = {}) {
  const position = row.position;
  const fullFactors = FULL_FACTORS[position] ?? [];
  const d6NonComp = (D6_NEW_FACTORS[position] ?? []).filter(f => f !== 'compBlend');
  const envelopeMembers = new Set([...ENVELOPE_FACTORS, ...ENVELOPE_FACTORS_D6_ADDITIONS]);
  const hold = new Set(holdAtOne);

  let innerLogSum = 0, outerLogSum = 0;
  for (const factor of [...fullFactors, ...d6NonComp]) {
    if (hold.has(factor)) continue;
    const f = row.multipliers?.[factor];
    if (f == null) continue;
    const term = Math.log(f);
    if (envelopeMembers.has(factor)) innerLogSum += term; else outerLogSum += term;
  }
  const inner = clamp(Math.exp(innerLogSum), FIT_COMBINED_CLAMP[0], FIT_COMBINED_CLAMP[1]);
  const outer = Math.exp(outerLogSum);
  const outerClamped = clamp(row.anchorBasePPG * outer * inner, FIT_OUTER_CLAMP[0], FIT_OUTER_CLAMP[1]);

  const hasCompBlend = (D6_NEW_FACTORS[position] ?? []).includes('compBlend');
  const compBlendFactor = (hasCompBlend && !hold.has('compBlend')) ? (row.multipliers?.compBlend ?? 1.0) : 1.0;
  const predicted = clamp(outerClamped * compBlendFactor, FIT_OUTER_CLAMP[0], FIT_OUTER_CLAMP[1]);

  return { predicted, preCompBlend: outerClamped };
}

// §D item 1 — the "optimism constant": median/mean outcome/fullPipelinePPG.
export function computeOptimismStats(rows, opts = {}) {
  const { holdAtOne = [] } = opts;
  const ratios = [];
  for (const row of rows) {
    const { predicted } = predictFullPipeline(row, { holdAtOne });
    if (predicted > 0 && Number.isFinite(row.outcomePPG)) ratios.push(row.outcomePPG / predicted);
  }
  const n = ratios.length;
  const mean = n > 0 ? ratios.reduce((a, b) => a + b, 0) / n : null;
  return { n, medianRatio: median(ratios), meanRatio: mean };
}

// Decision (Session 1, 2026-09-06) — the guard that makes the analogue
// framing a measurement, not an argument. Run computeOptimismStats twice per
// position: once with the full stack, once holding teamOffense/age/depth (the
// three named live-state divergences) at 1.0. Must run, and pass, BEFORE any
// other §D/§E output — the caller (runFullPipeline) enforces this ordering.
export const SENSITIVITY_STEP = 0.02; // the sweep's own step size (§D item 2 / Decision)
export const SENSITIVITY_HOLD_FACTORS = ['teamOffense', 'age', 'depth'];

export function runSensitivityCheck(rowsByPosition) {
  const perPosition = {};
  let allPass = true;
  for (const [position, rows] of Object.entries(rowsByPosition)) {
    const full = computeOptimismStats(rows, { holdAtOne: [] });
    const held = computeOptimismStats(rows, { holdAtOne: SENSITIVITY_HOLD_FACTORS });
    const absDiff = (full.medianRatio != null && held.medianRatio != null)
      ? Math.abs(full.medianRatio - held.medianRatio) : null;
    const pass = absDiff != null && absDiff <= SENSITIVITY_STEP;
    if (!pass) allPass = false;
    perPosition[position] = { full, held, absDiff, pass };
  }
  return { perPosition, allPass };
}

// §D item 2 — MAE of pipeline vs pipeline*c, c in [0.80..1.00] step 0.02.
export const OPTIMISM_C_GRID = [0.80, 0.82, 0.84, 0.86, 0.88, 0.90, 0.92, 0.94, 0.96, 0.98, 1.00];

export function computeMaeSweep(rows, opts = {}) {
  const { holdAtOne = [], cGrid = OPTIMISM_C_GRID } = opts;
  const preds = rows
    .map(row => ({ predicted: predictFullPipeline(row, { holdAtOne }).predicted, actual: row.outcomePPG }))
    .filter(p => Number.isFinite(p.predicted) && Number.isFinite(p.actual));
  return cGrid.map(c => {
    const n = preds.length;
    const mae = n > 0 ? preds.reduce((s, p) => s + Math.abs(p.predicted * c - p.actual), 0) / n : null;
    return { c, n, mae };
  });
}

// §D item 3 — shrinkage sweep: anchor' = (1-k)*anchor + k*positionMean, k in
// {0, 0.1, 0.2, 0.3}. positionMean is computed over the SAME row set passed
// in (the caller's own population — e.g. one qualifying-season tier), never
// a global constant, so it composes correctly with §D item 4's breakdown.
export const SHRINKAGE_K_GRID = [0, 0.1, 0.2, 0.3];

export function computeShrinkageSweep(rows, opts = {}) {
  const { holdAtOne = [], kGrid = SHRINKAGE_K_GRID } = opts;
  const anchors = rows.map(r => r.anchorBasePPG).filter(v => Number.isFinite(v));
  const positionMean = anchors.length > 0 ? anchors.reduce((a, b) => a + b, 0) / anchors.length : null;
  if (positionMean == null) return kGrid.map(k => ({ k, n: 0, mae: null }));

  return kGrid.map(k => {
    const preds = rows.map(row => {
      if (!(row.anchorBasePPG > 0)) return null;
      const shrunkAnchor = (1 - k) * row.anchorBasePPG + k * positionMean;
      const scale = shrunkAnchor / row.anchorBasePPG;
      const { predicted } = predictFullPipeline(row, { holdAtOne });
      const shrunkPredicted = clamp(predicted * scale, FIT_OUTER_CLAMP[0], FIT_OUTER_CLAMP[1]);
      return { predicted: shrunkPredicted, actual: row.outcomePPG };
    }).filter(p => p && Number.isFinite(p.predicted) && Number.isFinite(p.actual));
    const n = preds.length;
    const mae = n > 0 ? preds.reduce((s, p) => s + Math.abs(p.predicted - p.actual), 0) / n : null;
    return { k, n, mae };
  });
}

// §D item 4 — breakdown by qualifying-season count, already on the row.
export const QUALIFYING_TIERS = ['1-2', '3-4', '5+'];
export function qualifyingTierOf(row) {
  const n = row.qualifyingSeasons?.length ?? 0;
  if (n <= 2) return '1-2';
  if (n <= 4) return '3-4';
  return '5+';
}
export function groupByQualifyingTier(rows) {
  const groups = { '1-2': [], '3-4': [], '5+': [] };
  for (const row of rows) groups[qualifyingTierOf(row)].push(row);
  return groups;
}

function maeOf(preds) {
  const n = preds.length;
  return n > 0 ? preds.reduce((s, p) => s + Math.abs(p.predicted - p.actual), 0) / n : null;
}

// §E factor pruning — per-factor ablation, held at 1.0, ΔMAE/ΔSpearman vs
// shipped (every factor present). Fix pass 1 item 2 / the Decision:
// `qbQuality` is EXCLUDED entirely (empty eligible window — always neutral by
// construction, so it would always look prunable for a reason unrelated to
// signal). `teamOffense`/`age`/`depth` are likewise excluded from the pruning
// verdict (the Decision: reported separately as not-gradable) — they still
// CONTRIBUTE to fullPipelinePPG (predictFullPipeline always composes them);
// they are simply never proposed for pruning.
export const ABLATION_GRADABLE_FACTORS = {
  QB: ['momentum', 'regression', 'trajectory', 'rzUsage', 'efficiency', 'compBlend'],
  RB: ['momentum', 'regression', 'trajectory', 'shareTrend', 'snapShare', 'rzUsage', 'teamRzShare', 'efficiency', 'compBlend'],
  WR: ['momentum', 'regression', 'trajectory', 'shareTrend', 'snapShare', 'rzUsage', 'teamRzShare', 'efficiency', 'compBlend'],
  TE: ['momentum', 'regression', 'trajectory', 'shareTrend', 'snapShare', 'rzUsage', 'teamRzShare', 'efficiency', 'compBlend'],
};
export const NOT_GRADABLE_FACTORS = ['teamOffense', 'age', 'depth', 'qbQuality'];

export function runAblation(rows, position) {
  const factors = ABLATION_GRADABLE_FACTORS[position] ?? [];
  const shipped = rows
    .map(row => ({ predicted: predictFullPipeline(row, {}).predicted, actual: row.outcomePPG }))
    .filter(p => Number.isFinite(p.predicted) && Number.isFinite(p.actual));
  const shippedMae = maeOf(shipped);
  const shippedSpearman = shipped.length >= 10 ? spearman(shipped.map(p => p.predicted), shipped.map(p => p.actual)) : null;

  const perFactor = {};
  for (const factor of factors) {
    const held = rows
      .map(row => ({ predicted: predictFullPipeline(row, { holdAtOne: [factor] }).predicted, actual: row.outcomePPG }))
      .filter(p => Number.isFinite(p.predicted) && Number.isFinite(p.actual));
    const heldMae = maeOf(held);
    const heldSpearman = held.length >= 10 ? spearman(held.map(p => p.predicted), held.map(p => p.actual)) : null;
    perFactor[factor] = {
      n: held.length,
      maeWithout: heldMae, maeShipped: shippedMae,
      dMae: (heldMae != null && shippedMae != null) ? heldMae - shippedMae : null,
      spearmanWithout: heldSpearman, spearmanShipped: shippedSpearman,
      dSpearman: (heldSpearman != null && shippedSpearman != null) ? heldSpearman - shippedSpearman : null,
    };
  }
  return { position, n: shipped.length, shippedMae, shippedSpearman, perFactor };
}

// §E Step 4 — regressionFactor's up-side branches (outlierRatio<0.85 -> raw
// bucket 1.05/1.12, a bounce-back-UP correction for a player who underperformed
// their own career average). "Removing" them forces regressionFactor to 1.0
// for exactly those rows, leaving every other factor and every other
// regression bucket untouched. Reconstructs from PPG history alone — no live
// state, no divergence — proceeds unconditionally, unaffected by the
// sensitivity gate (§E's own text).
//
// injuryPredicate: DEVIATION, stated — the repo's hand-authored
// enrichment/injuries.json has ZERO entries in this checkout (nothing
// captured yet), so it cannot gate anything. The proxy used instead is the
// row's own predictor-year `dnpWeeks >= 3`, matching the app's OWN documented
// convention (README.md: "An absence run >=3 weeks is suggestive of injury
// but not labelled as such"). Reported as a proxy, not as real injury data.
//
// D-17 — this verdict is relative to the assembly's own regression model:
// under 'step4-upside', RB/WR/TE have no up-side left to remove (ΔMAE is 0 by
// construction), and only QB measures the retained up-side; under 'legacy'
// every position's up-side is graded, as before this versioning.
export function runStep4Verdict(rows, opts = {}) {
  const { injuryPredicate = null, bootstrap = null } = opts;

  function outlierRatioOf(row) {
    const qs = row.qualifyingSeasons ?? [];
    if (qs.length === 0) return null;
    const meanPPG = qs.reduce((a, s) => a + s.ppg, 0) / qs.length;
    const lastPPG = qs[qs.length - 1].ppg;
    return lastPPG / Math.max(meanPPG, 1);
  }

  function noUpsidePredicted(row) {
    const ratio = outlierRatioOf(row);
    if (ratio == null || !(ratio < 0.85) || row.multipliers?.regression == null) {
      return predictFullPipeline(row, {}).predicted;
    }
    const patched = { ...row, multipliers: { ...row.multipliers, regression: 1.0 } };
    return predictFullPipeline(patched, {}).predicted;
  }

  // D-17 — clustered bootstrap over per-row diffs (|noUpside-actual| -
  // |shipped-actual|), resampling by sleeperId cluster. The statistic is a
  // mean of per-row diffs, while dMae above is maeOf(noUpside) - maeOf(shipped)
  // — the two are NOT bit-identical beyond one row (dMae is a difference of
  // means of absolute errors, not a mean of differences), so never compare
  // them with === except on a one-row set.
  function bootstrapCI(rowSet) {
    if (!bootstrap) return null;
    const { resamples, seed } = bootstrap;

    const paired = [];
    for (const row of rowSet) {
      const shippedP = predictFullPipeline(row, {}).predicted;
      const noUpsideP = noUpsidePredicted(row);
      const actual = row.outcomePPG;
      if (!Number.isFinite(shippedP) || !Number.isFinite(noUpsideP) || !Number.isFinite(actual)) continue;
      if (row.sleeperId == null) throw new Error('[panel] runStep4Verdict bootstrap: a paired row lacks sleeperId');
      paired.push({ sleeperId: row.sleeperId, diff: Math.abs(noUpsideP - actual) - Math.abs(shippedP - actual) });
    }
    if (paired.length === 0) return null;

    const clusterOrder = [];
    const clusterMap = new Map();
    for (const { sleeperId, diff } of paired) {
      if (!clusterMap.has(sleeperId)) { clusterMap.set(sleeperId, []); clusterOrder.push(sleeperId); }
      clusterMap.get(sleeperId).push(diff);
    }
    const clusters = clusterOrder.map(id => clusterMap.get(id));
    ...
    for (let r = 0; r < resamples; r++) {
      ...
      for (let c = 0; c < clusters.length; c++) {
        ...
        for (const diff of cluster) { sum += diff; n++; }
      }
      stats.push(n > 0 ? sum / n : 0);
    }
    stats.sort((a, b) => a - b);
    const R = stats.length;
    const ci95 = [stats[Math.floor(0.025 * R)], stats[Math.ceil(0.975 * R) - 1]];
    const pNegative = stats.filter(v => v < 0).length / R;

    return { resamples, seed, clusters: clusters.length, ci95, pNegative };
  }

  function summarize(rowSet) {
    const shipped = rowSet
      .map(row => ({ predicted: predictFullPipeline(row, {}).predicted, actual: row.outcomePPG }))
      .filter(p => Number.isFinite(p.predicted) && Number.isFinite(p.actual));
    const noUpside = rowSet
      .map(row => ({ predicted: noUpsidePredicted(row), actual: row.outcomePPG }))
      .filter(p => Number.isFinite(p.predicted) && Number.isFinite(p.actual));
    const shippedMae = maeOf(shipped);
    const noUpsideMae = maeOf(noUpside);
    const shippedSp = shipped.length >= 10 ? spearman(shipped.map(p => p.predicted), shipped.map(p => p.actual)) : null;
    const noUpsideSp = noUpside.length >= 10 ? spearman(noUpside.map(p => p.predicted), noUpside.map(p => p.actual)) : null;
    return {
      n: shipped.length, shippedMae, noUpsideMae,
      dMae: (shippedMae != null && noUpsideMae != null) ? noUpsideMae - shippedMae : null,
      shippedSpearman: shippedSp, noUpsideSpearman: noUpsideSp,
      dSpearman: (shippedSp != null && noUpsideSp != null) ? noUpsideSp - shippedSp : null,
      bootstrap: bootstrapCI(rowSet),
    };
  }

  const overall = summarize(rows);
  const injuryGated = injuryPredicate ? summarize(rows.filter(injuryPredicate)) : null;
  return { overall, injuryGated };
}

// ─── Rookie panel (§E) — the reduced-scope reconstruction (see the deviation ──
// ─── note on reconstructRookieProjection, lib/projectionFactors.mjs). ─────────
//
// Rookie-path predicate mirrors the app's own routing test (seasonProjection.js
// :310, "qualifying.length===0 || yearsExp<=1") via the same retrospective
// proxy already established for the veteran panel's R1 exclusion: no
// qualifying (gp>=8) season anywhere <=Y, OR no season-totals appearance at
// all before Y. Pure; positionOf/birthdateOf/draftInfoOf are pid[,year]->value
// callbacks the caller (scripts/panel-run.mjs) supplies.
// ── D-8/D-9/D-12/D-13 (rookie-outcome-panels.md) ────────────────────────────
//
// rookiePathStateAt — §1 Q1(b): the single extracted definition of "is this
// pid/Y pair a rookie-path row", used by BOTH enumerators below. A byte-
// identical extraction of assembleRookiePanel's former inline predicate —
// see §6 test 10 for the drift check against attachFactorMultipliers'
// separate (unrefactored, veteran-panel) exclusion spelling.
export function rookiePathStateAt(pid, Y, { totalsByYear, ppgByYear, fromSeason = HISTORY_FLOOR }) {
  let hasQualifying = false;
  for (let s = fromSeason; s <= Y; s++) {
    const { ppg, gamesPlayed } = computeSeasonPoints(pid, ppgByYear[s]);
    if (Number.isFinite(ppg) && gamesPlayed >= 8) { hasQualifying = true; break; }
  }
  let earliestAppearance = null;
  for (let s = fromSeason; s < Y; s++) {
    if (totalsByYear[s]?.[pid] != null) { earliestAppearance = s; break; }
  }
  const isRookiePath = !hasQualifying || earliestAppearance == null;
  return { hasQualifying, earliestAppearance, isRookiePath };
}

// The six-game boundary is fixed and independent of assembleRookiePanel's
// gate (§2.2c) — it stays 6 whatever minOutcomeGames does.
export const OUTCOME_PLAYED_THRESHOLD = 6;

// classifyRookieOutcome — §2.2c. Pure. Reads the outcome map (so in-basis and
// half_ppr both reach it, per F4/F5) plus the raw season-totals row for
// dnpWeeks only, plus the outcome-year roster to split "absent" into three
// states (F6/F11).
export function classifyRookieOutcome({ pid, outcomeYear, ppgByYear, totalsByYear, rosterByYear }) {
  const outcomesForYear = ppgByYear?.[outcomeYear];
  const present = !!(outcomesForYear && outcomesForYear.has(String(pid)));
  const { gamesPlayed: outcomeGames, totalPts: outcomeTotalPts, ppg: outcomePPG } =
    computeSeasonPoints(pid, outcomesForYear);

  let outcomeClass;
  let dnpWeeks = null;
  if (present) {
    if (outcomeGames >= OUTCOME_PLAYED_THRESHOLD) outcomeClass = 'played6plus';
    else if (outcomeGames >= 1) outcomeClass = 'played1to5';
    else outcomeClass = 'rosteredZero';
    dnpWeeks = totalsByYear?.[outcomeYear]?.[pid]?.dnpWeeks ?? null;
  } else {
    const rosterFile = rosterByYear?.[outcomeYear];
    if (!rosterFile) outcomeClass = 'absentNoRosterFile';
    else if (rosterFile.players?.[pid]) outcomeClass = 'absentOnRoster';
    else outcomeClass = 'absentOffRoster';
  }

  return { outcomeClass, outcomeGames, outcomeTotalPts, outcomePPG, dnpWeeks };
}

// nflDraftTier -> draftGroup, per §0 item 1's r1/day2/day3 grouping.
const TIER_TO_DRAFT_GROUP = {
  'top-3': 'r1', 'top-8': 'r1', 'r1-mid': 'r1', 'r1-late': 'r1',
  r2: 'day2', r3: 'day2',
  r4: 'day3', r5: 'day3', r6: 'day3', r7: 'day3',
};

function resolveDraftGroup(draftInfo, nflDraftTier) {
  if (draftInfo?.undrafted === true) return 'undrafted';
  if (nflDraftTier && TIER_TO_DRAFT_GROUP[nflDraftTier]) return TIER_TO_DRAFT_GROUP[nflDraftTier];
  return 'unknown';
}

// enumerateEntryCohortRows — §2.2g. Pure. Walks each valid entrant forward
// from its draft ("entry") year, one row per target season, until the player
// leaves the rookie path (rookiePathStateAt) or has back-to-back zero-game
// seasons (games-with-absence-as-zero, per gamesOf below — this is the
// reading §5 risk 1 depends on). `entrantsBySleeper` is a pid-keyed map of
// { draftYear, draftRound, draftPick, undrafted, position } — position
// already resolved via the season-independent crosswalk by the caller
// (§2.3a: a player must carry one position across a whole cohort). The
// returned array carries a non-enumerable-free `invalidEntryYear` count
// (F10's draftYear<=0 sentinel) as an extra property, read by
// assembleRookiePanel. Fix pass 1 item 6: the counter is position-dependent
// and year-range-independent (position filtered first) — the number a
// verdict reader needs is "rows this cohort lost to the sentinel" (13 at
// panel positions), not "sentinel entries in the whole crosswalk" (45).
export function enumerateEntryCohortRows({
  entrantsBySleeper, totalsByYear, ppgByYear,
  fromEntryYear, toEntryYear, fromTarget, toTarget,
}) {
  const rows = [];
  let invalidEntryYear = 0;

  const gamesOf = (pid, s) => {
    const rec = totalsByYear?.[s]?.[pid];
    return rec ? (rec.gamesPlayed ?? 0) : 0;
  };

  for (const [sleeperId, entrant] of Object.entries(entrantsBySleeper ?? {})) {
    ...
    const entryYear = draftYear;
    for (let T = Math.max(entryYear, fromTarget); T <= toTarget; T++) {
      const state = rookiePathStateAt(sleeperId, T - 1, { totalsByYear, ppgByYear });
      if (state.hasQualifying) break;
      if (T - entryYear >= 2 && gamesOf(sleeperId, T - 1) === 0 && gamesOf(sleeperId, T - 2) === 0) break;

      const experienceYears = T - entryYear;
      const experienceBucket = experienceYears === 0 ? '0' : experienceYears === 1 ? '1' : '2+';
      rows.push({ sleeperId, entryYear, targetSeason: T, experienceYears, experienceBucket });
    }
  }

  rows.invalidEntryYear = invalidEntryYear;
  return rows;
}

// assembleRookiePanel — §2.2d. Default options reproduce the committed
// backtests/2026-09-06-fullpipeline-panel.json's rookiePanel rows/coverage
// byte-for-byte (§6 test 1) — this is the gate on the whole slice; the
// season-presence branch below is a deliberate byte-identical carry-forward
// of the pre-D-8/D-9/D-12/D-13 logic, not a rewrite.
export function assembleRookiePanel({
  totalsByYear, ppgByYear, positionOf, birthdateOf, draftInfoOf, fromYear, toYear,
  minOutcomeGames = 6,
  enumerator = 'season-presence',
  entrantsBySleeper = null,
  debutOnly = false,
  rosterByYear = null,
  predictor = reconstructRookieProjection,
}) {
  if (minOutcomeGames !== 6 && minOutcomeGames !== null) {
    throw new Error(`[rookie-panel] minOutcomeGames must be 6 or null, got ${JSON.stringify(minOutcomeGames)}`);
  }
  if (enumerator !== 'season-presence' && enumerator !== 'entry-cohort') {
    throw new Error(`[rookie-panel] unknown enumerator '${enumerator}'`);
  }
  if (enumerator === 'entry-cohort' && entrantsBySleeper == null) {
    throw new Error('[rookie-panel] entrantsBySleeper is required when enumerator is entry-cohort');
  }

  const rows = [];
  const coverage = {
    assembled: 0, surviving: 0, drops: {}, hitCapCount: 0,
    byOutcomeClass: {}, byCellOutcomeClass: {}, byRungCell: {},
    byExperience: {}, invalidEntryYear: 0,
    // §3.2 — provenance field, present on every assembly even when
    // `candidates` is empty; overwritten (to the same value) per row below.
    predictorCorrections: [],
  };

  const bump = (target, key) => { target[key] = (target[key] ?? 0) + 1; };
  const bumpRung = (key, games) => {
    const cell = (coverage.byRungCell[key] ??= { n: 0, sumGames: 0 });
    cell.n++;
    cell.sumGames += games;
  };

  // ── candidate enumeration (position-filtered; assembled counts these) ────
  let candidates;
  if (enumerator === 'season-presence') {
    candidates = [];
    for (let Y = fromYear; Y <= toYear; Y++) {
      const seasonTotals = totalsByYear[Y];
      if (!seasonTotals) continue;
      for (const pid of Object.keys(seasonTotals)) {
        if (isTeamAggregateId(pid)) continue;
        const position = positionOf(pid, Y);
        if (!PANEL_POSITIONS.includes(position)) continue;
        const { isRookiePath } = rookiePathStateAt(pid, Y, { totalsByYear, ppgByYear });
        if (!isRookiePath) continue;
        candidates.push({ pid, position, predictorYear: Y, outcomeYear: Y + 1 });
      }
    }
  } else {
    const cohortRows = enumerateEntryCohortRows({
      entrantsBySleeper, totalsByYear, ppgByYear,
      fromEntryYear: fromYear, toEntryYear: toYear,
      fromTarget: fromYear, toTarget: toYear,
    });
    coverage.invalidEntryYear = cohortRows.invalidEntryYear ?? 0;
    const filtered = debutOnly ? cohortRows.filter(r => r.targetSeason === r.entryYear) : cohortRows;
    candidates = [];
    for (const r of filtered) {
      const position = positionOf(r.sleeperId, r.entryYear);
      if (!PANEL_POSITIONS.includes(position)) continue;
      candidates.push({
        pid: r.sleeperId, position, predictorYear: r.entryYear, outcomeYear: r.targetSeason,
        entryYear: r.entryYear, targetSeason: r.targetSeason,
        experienceYears: r.experienceYears, experienceBucket: r.experienceBucket,
      });
    }
  }

  for (const cand of candidates) {
    const { pid, position, predictorYear: Y, outcomeYear } = cand;
    coverage.assembled++;

    const draftInfo = draftInfoOf(pid) ?? {};
    const birthdate = birthdateOf(pid);
    const birthYear = typeof birthdate === 'string' ? parseInt(birthdate.slice(0, 4), 10) : null;
    const ageAtDraft = (draftInfo.draftYear != null && birthYear != null) ? draftInfo.draftYear - birthYear : null;

    const proj = predictor({
      position, ageAtDraft,
      draftRound: draftInfo.draftRound ?? null,
      draftPick: draftInfo.draftPick ?? null,
    });

    // §1 Q4(b) — the re-fit-trap guard. Unconditional; no opt-out parameter.
    // rookie-mirror.md §3.1: the three correction tokens below
    // (rookieCalibration, rookieGames, rookieCeiling — ROOKIE_CORRECTIONS in
    // lib/rookieMirror.mjs is the authoritative copy) exist here as LITERALS,
    // deliberately, not an import — importing lib/rookieMirror.mjs into this
    // file would put the mirror one edge inside the fit path's closure, which
    // test/rookie-mirror.test.mjs's import-graph guard (T-RM1) forbids. A
    // future edit that imports ROOKIE_CORRECTIONS here reds T-RM1; the fix is
    // to keep the literal in sync (T-RM14 asserts the two copies agree), not
    // to exempt the import.
    if (!Array.isArray(proj.appliedCorrections)) {
      throw new Error(
        ...
        'explicit declaration that the UNCORRECTED stack ran (CR-15; see lib/rookieMirror.mjs and ' +
    ...
    if (proj.appliedCorrections.length) {
      throw new Error(
        ...
        'a fit panel must run the UNCORRECTED stack (CR-15 rookie mirror — rookieCalibration, ' +
        'rookieGames, rookieCeiling — belongs in lib/rookieMirror.mjs\'s reconstructShippedRookieProjection; ' +
        'see .claude/tasks/rookie-outcome-panels.md §1 Q4)'
      );
    }
    // §3.2 — provenance, not a second guard. By the time this line runs the
    // guard above has already refused any non-empty declaration, so this is
    // always []; it exists so a human reading the committed panel JSON can
    // see predictorCorrections without re-deriving it. No assertion anywhere
    // checks that it is empty (there is no assembler seam to stub and no
    // assertion for it to drive — the per-row throw above already fires
    // before this line, so an assertion here could never catch anything the
    // guard above did not already catch).
    coverage.predictorCorrections = [...proj.appliedCorrections];

    const outcome = classifyRookieOutcome({ pid, outcomeYear, ppgByYear, totalsByYear, rosterByYear });
    const draftGroup = resolveDraftGroup(draftInfo, proj.nflDraftTier);

    // D-9/D-12 coverage — computed for every assembled row, gate-independent.
    bump(coverage.byOutcomeClass, outcome.outcomeClass);
    const cellKey = `${draftGroup}|${position}`;
    bump((coverage.byCellOutcomeClass[cellKey] ??= {}), outcome.outcomeClass);

    if (enumerator === 'season-presence') {
      const draftYear = draftInfo.draftYear;
      let bucket;
      if (draftYear == null || draftYear <= 0) bucket = 'noYear';
      else {
        const diff = Y - draftYear;
        bucket = diff < 0 ? 'negative' : diff >= 3 ? '3+' : String(diff);
      }
      bump(coverage.byExperience, bucket);
    } else {
      bump(coverage.byExperience, cand.experienceBucket);
      if (draftGroup !== 'unknown') {
        bumpRung(`${draftGroup}|${position}|${cand.experienceBucket}`, outcome.outcomeGames);
        bumpRung(`${draftGroup}|${cand.experienceBucket}`, outcome.outcomeGames);
        bumpRung(`${draftGroup}|${position}`, outcome.outcomeGames);
        bumpRung(`${draftGroup}`, outcome.outcomeGames);
      } else {
        bumpRung(`U|${position}|${cand.experienceBucket}`, outcome.outcomeGames);
        bumpRung(`U|${position}`, outcome.outcomeGames);
      }
    }

    // ── the gate — 6 or null only (§2.2d) ────────────────────────────────
    if (minOutcomeGames === 6) {
      if (outcome.outcomePPG == null || !Number.isFinite(outcome.outcomePPG) || outcome.outcomeGames < minOutcomeGames) {
        coverage.drops.noOutcome = (coverage.drops.noOutcome ?? 0) + 1;
        continue;
      }
    }

    coverage.surviving++;
    if (proj.hitCap) coverage.hitCapCount++;

    const row = {
      sleeperId: pid, position, predictorYear: Y,
      draftYear: draftInfo.draftYear ?? null, draftRound: draftInfo.draftRound ?? null, draftPick: draftInfo.draftPick ?? null,
      projectedPPG: proj.projectedPPG, hitCap: proj.hitCap, nflDraftTier: proj.nflDraftTier,
      outcomePPG: outcome.outcomePPG,
      outcomeClass: outcome.outcomeClass, outcomeGames: outcome.outcomeGames, outcomeTotalPts: outcome.outcomeTotalPts,
      dnpWeeks: outcome.dnpWeeks, draftGroup,
    };
    if (enumerator === 'entry-cohort') {
      row.entryYear = cand.entryYear;
      row.targetSeason = cand.targetSeason;
      row.experienceYears = cand.experienceYears;
      row.experienceBucket = cand.experienceBucket;
    }
    rows.push(row);
  }

  // §2.2f — "When minOutcomeGames is null, drops is {} and surviving ===
  // assembled; assert that relationship in code, do not leave it implicit."
  if (minOutcomeGames === null) {
    if (coverage.surviving !== coverage.assembled || Object.keys(coverage.drops).length !== 0) {
      throw new Error('[rookie-panel] invariant violated: ungated assembly must have surviving === assembled and no drops');
    }
  }

  for (const key of Object.keys(coverage.byRungCell)) {
    const cell = coverage.byRungCell[key];
    cell.meanOutcomeGames = cell.n > 0 ? cell.sumGames / cell.n : null;
    cell.meanOutcomeGamesRounded = cell.meanOutcomeGames == null ? null : Math.round(cell.meanOutcomeGames);
    delete cell.sumGames;
  }
