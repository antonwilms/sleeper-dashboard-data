...
export const PANEL_DEFAULTS = {
  fromYear: 2020, toYear: 2024,          // predictor years; outcomes Y+1 (flip fromYear→2013 post-R1-SNAPS;
                                          // see CORRUPT_PREDICTOR_SEASONS in ./backtest.mjs — 2016 stays
                                          // excluded from airYardsShare regardless of this bound)
...
export function buildTeamTotalsForSeason(totalsSeason, season, teamOf) {
  ...
  for (const [pid, rec] of Object.entries(totalsSeason)) {
    ...
    if (!totals[team]) totals[team] = { recTgt: 0, rushAtt: 0, recRzTgt: 0, rushRzAtt: 0, rec: 0 };
    const s = rec?.stats ?? {};
...
// teamRz pool's denominator only).
export function buildCohortPools(totalsSeason, positionOf, teamTotalsSeason) {
  const pools = {
    QB: { snap: [], rz: [], teamRz: [] },
    RB: { snap: [], rz: [], teamRz: [] },
    WR: { snap: [], rz: [], teamRz: [] },
...
export function attachFactorMultipliers(rows, ctx) {
  const { totalsByYear, teamTotalsByYear, ppgByYear, advstatsByYear = {}, rosterByYear = {} } = ctx;
  ...
  const getPools = (Y) => {
    if (!poolsByYear[Y]) {
      const positionOf = (pid) => resolvePosition(pid, advstatsByYear[Y], rosterByYear[Y]);
      poolsByYear[Y] = buildCohortPools(totalsByYear[Y], positionOf, teamTotalsByYear[Y]);
    }
  ...
  for (const row of rows) {
    ...
    // ── Step 4: anchorBasePPG (null is structurally unreachable once R1
