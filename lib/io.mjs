 ...
 * Uses 2-space indentation and a trailing newline by default.
 *
 * @param {string} relPath
 * @param {*} value
 * @param {object} [opts]
 * @param {boolean} [opts.minify=false]  Write with no whitespace instead of the
 *   2-space default. Opt-in only — do not flip the default; ~50 call sites across
 *   18 files rely on the pretty-printed shape (manifest.json's hand-merged rebase
 *   workflow in particular). Callers: the season-totals writer and its migration.
 */
export function writeJsonStable(relPath, value, { minify = false } = {}) {
  const abs = repoPath(relPath);
  fs.mkdirSync(path.dirname(abs), { recursive: true });
  const json = minify ? JSON.stringify(value) : JSON.stringify(value, null, 2);
...
/**
 * Deep variant of sortObjectKeys — recursively sorts object keys at every nesting level
 * (array element order is left alone; only object key order is unordered here). Needed where
 * a *nested* key order is also insertion-order-derived and must not affect hash equality — the
 * CFBD pivot's `players` map is one example: both the top-level playerId keys and each
 * per-player record's statType keys are order-sensitive by default, so a shallow sort would
 * still let per-record key order cause a spurious mismatch (college-pivot-phase-b.md §1.4.2).
 */
export function deepSortKeys(value) {
  if (Array.isArray(value)) return value.map(deepSortKeys);
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.keys(value).sort().map(k => [k, deepSortKeys(value[k])]));
  }
  return value;
}
