 ...
 * @param {string|null} enrichmentDir  Absolute path to read enrichment files from.
 *   Defaults to null (reads from the repo-relative enrichment/ directory).
 *   Intended for isolated test fixtures only.
 */
export function validateAll(context = {}, enrichmentDir = null) {
  for (const type of ENRICHMENT_TYPES) {
    let payload;
    if (enrichmentDir) {
      const p = `${enrichmentDir}/${type}.json`;
      payload = fs.existsSync(p)
        ? JSON.parse(fs.readFileSync(p, 'utf8'))
        : { schemaVersion: 1, entries: [] };
    } else {
      payload = readEnrichment(type);
    }
