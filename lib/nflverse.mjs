 ...
 */
export function aggregateSnapCounts(csv, { positions = ['QB', 'RB', 'WR', 'TE', 'FB'] } = {}) {
  const lines = csv.replace(/\r\n/g, '\n').replace(/\r/g, '\n')
    .split('\n')
    .filter(l => l.trim());
  ...
  for (const [pfrId, p] of Object.entries(byPlayer)) {
    ...
    // Primary team: max offSnaps; tie → alphabetical (first).
    const primaryTeamEntry = teamEntries.reduce((best, cur) => {
      if (!best) return cur;
      const [bt, bv] = best;
      const [ct, cv] = cur;
      if (cv.offSnaps > bv.offSnaps) return cur;
      if (cv.offSnaps === bv.offSnaps && ct < bt) return cur;
      return best;
    }, null);
    const primaryTeam = primaryTeamEntry?.[0] ?? null;
    ...
    if (teamEntries.length > 1) {
      ...
      entry.teams = teamEntries
        .slice()
        .sort((a, b) => b[1].offSnaps - a[1].offSnaps)
        .map(([t]) => t);
    }
  ...
  return { byPfrId, season, rowCount };
}

/**
 * Build a pfr_id → sleeper_id reverse index from the playerids crosswalk's `bySleeper` map
 * (there is no natural pfrId-keyed index — `bySleeper` is sleeper_id-keyed). Keep-first on a
 * duplicate `pfrId` (unexpected upstream collision; arbitrary but deterministic).
 *
 * @param {object} bySleeper  { [sleeper_id]: { pfrId, ... } }  from playerids.json `.bySleeper`
...
export function pfrCrosswalkFromBySleeper(bySleeper) {
  ...
  for (const [sleeperId, entry] of Object.entries(bySleeper ?? {})) {
    const pfrId = entry?.pfrId;
    if (!pfrId || byPfr[pfrId]) continue;
    byPfr[pfrId] = sleeperId;
  }
  return byPfr;
}
