 ...
 */
async function fetchPage(page, dryRun = false) {
  ...
  if (!dryRun) console.log(`[ktc] Page ${page}: ${rows.length} div.onePlayer elements`);
...
export async function fetchKtcSnapshot({ dryRun = false } = {}) {
  ...
  for (let page = 0; page <= 9; page++) {
    ...
    let players;
    try {
      players = await fetchPage(page, dryRun);
    } catch (err) {
    ...
    if (!dryRun) console.log(`[ktc] Page ${page}: ${players.length} rows, ${newCount} new — total ${allPlayers.length}`);
