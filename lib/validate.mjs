...
export function validateNflSeason(totals, { year }) {
  ...
  // instead. On a complete season maxGames = 17 → threshold = 14, numerically identical to the
  // old fixed floor (backwards-compatibility is the point, not a side effect). Mid-season
  // maxGames = 5 → threshold = 2 — still catches a genuinely broken scrape, which yields a few
  // scattered players, not thirty clustered near the current leader.
...
import { MIN_ROSTER_IDS, MIN_PLAYERID_ROWS, MIN_ADVSTATS_ROWS, MIN_SCHEDULE_GAMES,
         MIN_PLAYERGAME_ROWS, MIN_TEAMCONTEXT_ROWS, MIN_OLINE_ROWS } from './nflverse.mjs';
