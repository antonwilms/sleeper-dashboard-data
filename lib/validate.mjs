 ...
 * ║  At least 3 of these should appear in the top-15 by value.    ║
 * ║  Update when the dynasty QB landscape shifts significantly;   ║
 * ║  a miss quarantines the KTC scrape (update-ktc.mjs).           ║
 * ╚══════════════════════════════════════════════════════════════════╝
 */
...
// At least 3 of these should appear in the KTC top-15 by dynasty value
// (checkKtcLandscape). A landscape heuristic, not a validity check: a miss
// quarantines the scrape (scripts/update-ktc.mjs) rather than throwing.
// Updated 2026-09-26 from ktc/snapshot-2026-09-21.json: top-10 QBs were Allen (1),
// Williams (8), Maye (9), Jackson (10); Burrow (18) and Daniels (20) are the next
// two. Hurts (30) and Mahomes (36) were dropped. The window is top 15, not 10: the
// top 10 holds only 3-4 QBs and Maye moved #9 → #14 in one week (live 2026-09-26),
// so a top-10 rule sat at exactly 3 with no headroom. Top 15 holds 4 (Allen,
// Williams, Jackson, Maye) on both the 9/21 snapshot and the live 9/26 scrape.
...
const KTC_LANDSCAPE_TOP_N = 15;
const KTC_MIN_SENTINELS = 3;
...
/**
 * Validates a KTC player snapshot array (per-row + count validity).
 * Throws with a descriptive message if any check fails. The top-15 QB landscape
 * heuristic is NOT here — see checkKtcLandscape (quarantines, never throws).
 *
...
/**
 * Landscape heuristic: at least KTC_MIN_SENTINELS known dynasty QBs
 * should sit in the top KTC_LANDSCAPE_TOP_N by value. Unlike validateKtc this never throws — a
 * miss can mean the QB landscape shifted (stale list) as easily as a bad scrape,
 * so the caller quarantines the snapshot instead of losing the week.
 *
 * @param {Array<{name: string, value: number}>} players  row-valid KTC snapshot
 * @returns {{ ok: boolean, matches: string[], top: string[], reason?: string }}
 */
export function checkKtcLandscape(players) {
  const top = players
    .slice()
    .sort((a, b) => b.value - a.value)
    .slice(0, KTC_LANDSCAPE_TOP_N)
    .map(p => p.name);

  const matches = KTC_TOP_QB_SENTINELS.filter(name => top.includes(name));
  if (matches.length >= KTC_MIN_SENTINELS) return { ok: true, matches, top };
  return {
    ok: false, matches, top,
    reason: `only ${matches.length} sentinel QBs in top ${KTC_LANDSCAPE_TOP_N} (${top.join(', ')}). ` +
      `Expected ≥ ${KTC_MIN_SENTINELS} of: ${KTC_TOP_QB_SENTINELS.join(', ')}. ` +
...
import { MIN_ROSTER_IDS, MIN_PLAYERID_ROWS, MIN_ADVSTATS_ROWS, MIN_SCHEDULE_GAMES,
         ...
         MIN_DEPTH_SEASON, MIN_DEPTH_ROWS, DEPTH_JOIN_RATE_MIN, MIN_DEPTH_QB1_TEAMS,
         DEPTH_SKILL_POSITIONS } from './nflverse.mjs';
...
// The draft fields (draft_round/draft_pick/draft_ovr) are deliberately NOT
// floor-gated — see doc below, finding 2. But "no floor" is not "no check":
// a ceiling catches the one failure mode a floor can't, without firing on the
// legitimate undrafted population. If upstream renames or reshapes
// draft_round, every row parses as undrafted and nothing else would notice.
const MAX_UNDRAFTED_RATE = 0.75; // measured 0.417 (2026-09-05); real UDFA share, wide margin below the ceiling
 ...
 * defence-in-depth (throws on clearly bad data that somehow passed the gate).
 *
 * Deliberately NOT floor-gated: `draft_round`/`draft_pick`/`draft_ovr` are null
 * for every undrafted player (42% of bySleeper, verified against
 * draft_picks.json — see parsePlayerIdsCsv doc), so a fill-rate floor on any
 * of them would fire on correct data forever. Their absence is instead caught
 * by a ceiling on the derived `undrafted` rate (throws above 0.75) — the
 * counterpart check for a format break that would mark every row undrafted.
 * `ktc_id` is 7% populated upstream (measured 2026-09-05) — real, not a
 * parser fault — so it is served but never gated.
 *
...
export function validatePlayerIds(ids, bySleeper) {
  ...
  // Ceiling counterpart to the deliberately-ungated draft fields (finding 2):
  // no floor, because ~42% legitimately undrafted; a ceiling still catches a
  // format break that would otherwise mark every row undrafted unnoticed.
  let undraftedCount = 0;
  for (const v of Object.values(bySleeper)) {
    if (v.undrafted) undraftedCount++;
  }
  const undraftedRate = sleeperCount ? undraftedCount / sleeperCount : 0;
  if (undraftedRate > MAX_UNDRAFTED_RATE) {
    throw new Error(
      `[validate] playerids: undrafted rate ${undraftedRate.toFixed(3)} above ceiling ` +
      `${MAX_UNDRAFTED_RATE} (${undraftedCount}/${sleeperCount}) — possible upstream draft_round format change.`
    );
  }
}
...
/**
 * Validates a re-keyed snaps players object (defence-in-depth after the season floor).
 *
 * `rowCount` here is the REAL MIN_SNAPS_ROWS gate — deliberately NOT enforced by
 * `runSeasonKeyedIngest`'s own sparsity skip (scripts/update-snaps.mjs passes it a low
 * `minRows` on purpose). Every season this family backfills is already complete/published,
 * so a shortfall here means a truncated fetch, not an unpublished season — that case belongs
 * in a hard throw, not a silent skip-and-continue (task finding 3/§C).
 *
 * @param {object} players   { [sleeper_id]: { pfrId, position, team, [traded, teams,]
 *                              games, offSnaps, teamOffSnaps, offPct } }  post-crosswalk
 * @param {object} opts
 * @param {number} opts.year
 * @param {number} opts.joinRate  mapped / (mapped + unmapped) over skill-position players
 */
export function validateSnaps(players, { year, joinRate }) {
  if (year < MIN_SNAPS_SEASON) {
    throw new Error(`[validate] snaps ${year}: season below MIN_SNAPS_SEASON=${MIN_SNAPS_SEASON} — refusing to ingest.`);
  }
  if (typeof joinRate === 'number' && joinRate < SNAPS_JOIN_RATE_MIN) {
    throw new Error(
      `[validate] snaps ${year}: pfrId join rate ${joinRate.toFixed(3)} below floor ` +
      `${SNAPS_JOIN_RATE_MIN} — possible crosswalk regression (D2 pfrId).`
    );
  }

  const entries = Object.values(players);
  const rowCount = entries.reduce((s, p) => s + (p.games || 0), 0);
  if (rowCount < MIN_SNAPS_ROWS) {
    throw new Error(
      `[validate] snaps ${year}: only ${rowCount} player-game rows — expected ≥ ${MIN_SNAPS_ROWS}. ` +
      `Possible truncated fetch (this season is already published — "not yet published" would ` +
      `have skipped upstream of this validator, not landed here with a partial row count).`
    );
  }

  // Finding 3 regression: this source is already historically team-coded — eraTeam() must
  // never touch it. Same three boundary pairs as validateTeamContext's era-domain guard.
  const teamsSeen = new Set();
  for (const p of entries) {
    if (p.team) teamsSeen.add(p.team);
    for (const t of p.teams || []) teamsSeen.add(t);
  }
  if (year <= 2015 && teamsSeen.has('LA'))
    throw new Error(`[validate] snaps ${year}: team 'LA' present — Rams are 'STL' at year ≤ 2015 in this already-historically-coded source.`);
  if (year >= 2016 && teamsSeen.has('STL'))
    throw new Error(`[validate] snaps ${year}: team 'STL' present — Rams are 'LA' from 2016 on.`);
  if (year <= 2016 && teamsSeen.has('LAC'))
    throw new Error(`[validate] snaps ${year}: team 'LAC' present — Chargers are 'SD' at year ≤ 2016.`);
  if (year >= 2017 && teamsSeen.has('SD'))
    throw new Error(`[validate] snaps ${year}: team 'SD' present — Chargers are 'LAC' from 2017 on.`);
  if (year <= 2019 && teamsSeen.has('LV'))
    throw new Error(`[validate] snaps ${year}: team 'LV' present — Raiders are 'OAK' at year ≤ 2019.`);
  if (year >= 2020 && teamsSeen.has('OAK'))
    throw new Error(`[validate] snaps ${year}: team 'OAK' present — Raiders are 'LV' from 2020 on.`);

  for (const p of entries) {
    if (p.offPct != null && (p.offPct < 0 || p.offPct > 1))
      throw new Error(`[validate] snaps ${year}: offPct out of [0,1] for pfrId ${p.pfrId} (=${p.offPct}).`);
  }

  const nf = findNonFinite(players);
  if (nf) throw new Error(`[validate] snaps ${year}: non-finite numeric at ${nf.path} (=${String(nf.value)}).`);
}

// ─── nflverse depth charts (both eras, 2013+) ────────────────────────────────

/**
 * Validates a joined nflverse/depth `weeks` object (defence-in-depth after the spine's own
 * near-inert sparsity skip — this family follows scripts/update-snaps.mjs's pattern: real
 * floors thrown here, not skipped-and-continued at the spine, so a legacy filter mistake can
 * never silently skip all twelve backfill seasons green).
 *
 * @param {object} weeks     { [week]: { [team]: { QB:[sleeperId|null,…], RB:[…], WR:[…], TE:[…],
 *                              [depthPositions], [dt, date] } } }  post-join (joinDepthToSleeper)
 * @param {object} opts
 * @param {number} opts.year
 * @param {number} opts.joinRate  mapped / (mapped + unmapped) over DISTINCT skill-position ids
 */
export function validateDepth(weeks, { year, joinRate }) {
  if (year < MIN_DEPTH_SEASON) {
    throw new Error(`[validate] depth ${year}: season below MIN_DEPTH_SEASON=${MIN_DEPTH_SEASON} — refusing to ingest.`);
  }
  if (typeof joinRate === 'number' && joinRate < DEPTH_JOIN_RATE_MIN) {
    throw new Error(
      `[validate] depth ${year}: skill-position join rate ${joinRate.toFixed(4)} below floor ` +
      `${DEPTH_JOIN_RATE_MIN} — possible crosswalk regression.`
    );
  }

  const week1 = weeks['1'];
  if (!week1) {
    throw new Error(`[validate] depth ${year}: no week 1 present — refusing to write.`);
  }

  // ≥30, not exactly 32 (measured 2026-09-06 against live source, beyond the task's four checked
  // seasons): 2017 has only 30 at week 1 REG — the MIA@TB week-1 game was postponed to week 11
  // that season (Hurricane Irma), so both teams' legacy depth-chart rows are genuinely absent
  // from week 1 upstream, not a parser fault. Mirrors MIN_DEPTH_QB1_TEAMS's own slack rather than
  // inventing a separate threshold.
  const teamKeys = Object.keys(week1);
  if (teamKeys.length > 32) {
    throw new Error(`[validate] depth ${year}: ${teamKeys.length} teams at week 1 — expected ≤ 32.`);
  }
  if (teamKeys.length < MIN_DEPTH_QB1_TEAMS) {
    throw new Error(`[validate] depth ${year}: only ${teamKeys.length} teams at week 1 — expected ≥ ${MIN_DEPTH_QB1_TEAMS}.`);
  }

  // Era-domain guard (the eraTeam regression trap, both directions — same three boundary pairs
  // as validateTeamContext/validateSnaps; this source is already historically team-coded, like
  // snaps — eraTeam() must never be applied to it, finding 4).
  if (year <= 2015 && teamKeys.includes('LA'))
    throw new Error(`[validate] depth ${year}: team 'LA' present — Rams are 'STL' at year ≤ 2015.`);
  if (year >= 2016 && teamKeys.includes('STL'))
    throw new Error(`[validate] depth ${year}: team 'STL' present — Rams are 'LA' from 2016 on.`);
  if (year <= 2016 && teamKeys.includes('LAC'))
    throw new Error(`[validate] depth ${year}: team 'LAC' present — Chargers are 'SD' at year ≤ 2016.`);
  if (year >= 2017 && teamKeys.includes('SD'))
    throw new Error(`[validate] depth ${year}: team 'SD' present — Chargers are 'LAC' from 2017 on.`);
  if (year <= 2019 && teamKeys.includes('LV'))
    throw new Error(`[validate] depth ${year}: team 'LV' present — Raiders are 'OAK' at year ≤ 2019.`);
  if (year >= 2020 && teamKeys.includes('OAK'))
    throw new Error(`[validate] depth ${year}: team 'OAK' present — Raiders are 'LV' from 2020 on.`);

  // Depth-1 QB gate (finding 5) — index 0, not "any QB present": joinDepthToSleeper keeps a
  // `null` at its original index for an unresolved entry rather than dropping it, specifically
  // so this check reads the true depth-1 slot rather than a promoted backup.
  let qb1Teams = 0;
  for (const team of teamKeys) {
    if ((week1[team].QB || [])[0]) qb1Teams++;
  }
  if (qb1Teams < MIN_DEPTH_QB1_TEAMS) {
    throw new Error(
      `[validate] depth ${year}: only ${qb1Teams} teams have a depth-1 QB at week 1 — expected ≥ ${MIN_DEPTH_QB1_TEAMS}.`
    );
  }

  // Row-count floor — the real MIN_DEPTH_ROWS gate (mirrors validateSnaps; the spine's own
  // sparsity skip is deliberately near-inert for this family, see scripts/update-depth.mjs).
  // Counts resolved (non-null) entries only, across every week.
  let rowCount = 0;
  for (const teams of Object.values(weeks)) {
    for (const posMap of Object.values(teams)) {
      for (const pos of DEPTH_SKILL_POSITIONS) {
        for (const id of posMap[pos] || []) if (id) rowCount++;
      }
    }
  }
  if (rowCount < MIN_DEPTH_ROWS) {
    throw new Error(
      `[validate] depth ${year}: only ${rowCount} joined skill-position rows — expected ≥ ${MIN_DEPTH_ROWS}. ` +
      'Possible truncated fetch (this season is already published — "not yet published" would ' +
      'have skipped upstream of this validator, not landed here with a partial row count).'
    );
  }

  const nf = findNonFinite(weeks);
  if (nf) throw new Error(`[validate] depth ${year}: non-finite numeric at ${nf.path} (=${String(nf.value)}).`);
}
