...
import { MIN_ROSTER_IDS, MIN_PLAYERID_ROWS, MIN_ADVSTATS_ROWS, MIN_SCHEDULE_GAMES,
         MIN_PLAYERGAME_ROWS, MIN_TEAMCONTEXT_ROWS } from './nflverse.mjs';
 ...
 *
 * @param {object} teams  { [teamAbbr]: { states: [...] } }  from aggregateOlineStates
 * @param {object} opts
 * @param {number} opts.year
 */
export function validateOline(teams, { year }) {
  ...
  for (const key of teamKeys) {
    for (const state of teams[key].states || []) {
      ...
      if (!state.ol || state.ol.length === 0) {
        throw new Error(`[validate] oline ${year}: ${key} state ${state.week} has empty ol[].`);
      }

      const seenSlotRank = new Set();
      for (const entry of state.ol) {
        if (!OLINE_SLOT_SET.has(entry.slot)) {
          throw new Error(`[validate] oline ${year}: ${key} state ${state.week} has invalid slot '${entry.slot}'.`);
        }
        if (!Number.isInteger(entry.rank) || entry.rank < 1) {
          throw new Error(`[validate] oline ${year}: ${key} state ${state.week} ${entry.slot} has invalid rank (=${entry.rank}).`);
        }
        if (!entry.name) {
          throw new Error(`[validate] oline ${year}: ${key} state ${state.week} ${entry.slot} rank ${entry.rank} has empty name.`);
        }
        const slotRankKey = `${entry.slot}|${entry.rank}`;
        if (seenSlotRank.has(slotRankKey)) {
          throw new Error(`[validate] oline ${year}: ${key} state ${state.week} has duplicate (slot,rank) ${slotRankKey}.`);
        }
