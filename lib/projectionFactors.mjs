 ...
 *   - the Step 4 regression bucket table is versioned (D-17): `resolveRegressionBucket` mirrors
 *     src/utils/seasonProjection.js's Step 4 bucket and `REGRESSION_UPSIDE_POSITIONS` at app 7b5b055,
 *     with two models (`legacy`, `step4-upside`).
...
// ─── Rookie panel (D6b, §E) — sleeper-dashboard's seasonProjection.js:41-240 ──
// ─── rookieProjection + resolveNflDraftFactor. ────────────────────────────────
//
// DEVIATION beyond finding 9's own "3 of 5, not 4" bound (disclosed in the
// D6b hand-back, not silently narrowed): this reconstruction implements TWO
// of the five real inputs — ageMult and nflDraftMultiplier — plus the
// baseline table. `ktcMult` and `collegeContribution` are BOTH always
// neutral here: `ktcMult` because KTC history starts 2026-05-18, after the
// entire 2013-2025 panel window (the same structural gap as `qbQuality`);
// `collegeContribution` because porting `computeCollegeMetrics` (a team-share
// "dominator" rating, a conference-multiplier lookup table, and three
// production-trend/final-year branches) is a genuinely separate sub-project
// this slice did not build — not merely "narrow coverage" the way finding 9
// describes the app's own 2017 college-data floor.
//
// D-8/D-9/D-12/D-13 (rookie-outcome-panels.md §1 Q4) — this function is now
// explicitly the UNCORRECTED FIT PREDICTOR, permanently. CR-15's rookie
// mirror (ROOKIE_CALIBRATION, DAY3_TIERS, the five-rung games ladder from
// the app's rookie-calibration.md / rookie-availability.md, and now also the
// ceiling from rookie-ceiling.md — three mechanisms, not two) does NOT land
// here — landing it here would put those constants one import from the fit
// path this function feeds, poisoning any re-fit with the corrections it is
// meant to validate. That mirror lives in `lib/rookieMirror.mjs`'s
// `reconstructShippedRookieProjection`, which COMPOSES this function's
// `cappedProduct` (never its `projectedPPG`, which is already rounded and
// clamped) — see rookie-mirror.md §2/§6.1 — and is kept out of this file's
// import closure by test/rookie-mirror.test.mjs's import-graph guard. See
// rookie-outcome-panels.md §1 Q4 for the full original reasoning.

export const ROOKIE_BASELINE_PPG = { QB: 13, RB: 9, WR: 7, TE: 5 };

// seasonProjection.js:41-70 resolveNflDraftFactor, ported verbatim (tier
// table + neutral sentinel on no draft match).
const NFL_DRAFT_TIERS = [
  { test: (round, pick) => round === 1 && pick <= 3, mult: 1.30, tier: 'top-3' },
  { test: (round, pick) => round === 1 && pick <= 8, mult: 1.18, tier: 'top-8' },
  { test: (round, pick) => round === 1 && pick <= 15, mult: 1.10, tier: 'r1-mid' },
  { test: (round) => round === 1, mult: 1.02, tier: 'r1-late' },
  { test: (round) => round === 2, mult: 0.92, tier: 'r2' },
  { test: (round) => round === 3, mult: 0.82, tier: 'r3' },
  { test: (round) => round === 4, mult: 0.74, tier: 'r4' },
  { test: (round) => round === 5, mult: 0.68, tier: 'r5' },
  { test: (round) => round === 6, mult: 0.62, tier: 'r6' },
];

export function reconstructNflDraftFactor(round, pick) {
  if (round == null || pick == null) return { nflDraftMultiplier: 1.0, nflDraftTier: null };
  for (const { test, mult, tier } of NFL_DRAFT_TIERS) {
    if (test(round, pick)) return { nflDraftMultiplier: mult, nflDraftTier: tier };
  }
  return { nflDraftMultiplier: 0.58, nflDraftTier: 'r7' };
}

// seasonProjection.js:83-95 age-bucket table. ageAtDraft = draftYear -
// birthYear — this reconstruction's retrospective analog of the app's
// `rookieAgeAtDraft = age - yearsExp` (bounded [18,28] there); computed
// directly from D2 birthdate + draftYear rather than approximated from a
// live "current age", so arguably MORE precise than the app's own version.
// null (missing birthdate or draft year) -> the app's own default-age(23)
// bucket (0.95), matching its `age ?? 23` fallback.
export function reconstructRookieAgeFactor(ageAtDraft) {
  if (ageAtDraft == null) return 0.95;
  if (ageAtDraft <= 21) return 1.15;
  if (ageAtDraft === 22) return 1.05;
  if (ageAtDraft === 23) return 0.95;
  return 0.82;
}

// seasonProjection.js:164-171 — clamp(product, 0.45, 1.85), the "1.85 cap".
export const ROOKIE_MULTIPLIER_CLAMP = [0.45, 1.85];

// rookie-outcome-panels.md §1 Q4(b) / §2.1a — the re-fit-trap guard's
// declaration. Frozen so a caller cannot mutate the shared reference.
export const EMPTY_CORRECTIONS = Object.freeze([]);

export function reconstructRookieProjection({ position, ageAtDraft, draftRound, draftPick }) {
  const baseline = ROOKIE_BASELINE_PPG[position] ?? 7;
  const ageMult = reconstructRookieAgeFactor(ageAtDraft);
  const ktcMult = 1.0; // always neutral this slice — see the deviation note above
  const collegeContribution = 1.0; // always neutral this slice — see the deviation note above
  const { nflDraftMultiplier, nflDraftTier } = reconstructNflDraftFactor(draftRound, draftPick);

  const rawProduct = ageMult * ktcMult * collegeContribution * nflDraftMultiplier;
  const cappedProduct = Math.max(ROOKIE_MULTIPLIER_CLAMP[0], Math.min(ROOKIE_MULTIPLIER_CLAMP[1], rawProduct));
  const projectedPPGRaw = Math.max(0, Math.min(40, baseline * cappedProduct));

  return {
    projectedPPG: Math.round(projectedPPGRaw * 10) / 10,
    ageMult, ktcMult, collegeContribution, nflDraftMultiplier, nflDraftTier,
    rawProduct, cappedProduct,
    hitCap: rawProduct > ROOKIE_MULTIPLIER_CLAMP[1] || rawProduct < ROOKIE_MULTIPLIER_CLAMP[0],
