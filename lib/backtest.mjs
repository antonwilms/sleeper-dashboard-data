 ...
 *
 * Empty since 2026-08-30 (reingest-2016.md): 2016's air-yards corruption was a stale
 * legacy-tag ingest, corrected by re-ingesting from the current stats_player tag — not a
 * permanent property of the season.
 ...
 * and lib/panel.mjs (candidates.airYardsShare) reads this and needs no further wiring.
 */
export const CORRUPT_PREDICTOR_SEASONS = {
};

export function isCorruptPredictorSeason(metric, year) {
  return (CORRUPT_PREDICTOR_SEASONS[metric] ?? []).includes(year);
}
...
export function buildCohortRows(advstatsY, totalsY, totalsY1, {
  ...
  for (const [sleeperId, player] of Object.entries(advstatsY.players)) {
    ...
    // snapShare — null for pre-2020 seasons (season-totals' own off_snp/
    // tm_off_snp are never populated before 2020). Revisited for D6a (finding
    // 3): this standalone D3 self-validation tool intentionally does NOT
    // widen alongside lib/panel.mjs — it reads season-totals only and has no
    // injectable loader for nflverse/snaps/<year>.json (D4), unlike
    // lib/panel.mjs's resolveSnapCounts fallback. The claim below remains
    // accurate for what THIS module reads; it is not a claim that snap data
