 ...
 * @param {number}  [opts.schemaVersion=1]  Per-file schema version (Phase 5 bumps nfl season totals to 2)
 */
export function updateManifestEntry({ path, recordCount, inProgress, schemaVersion = 1 }) {
 ...
 *
 * @param {object} opts
 * @param {string}  opts.path         Repo-relative data path, e.g. 'nfl/season-totals/2025.json'
 * @param {boolean} opts.inProgress   The new inProgress value
...
export function setManifestInProgress({ path, inProgress }) {
  ...
  if (existing.inProgress === inProgress) return false;

  manifest.files[path] = { ...existing, inProgress };
  manifest.generatedAt = new Date().toISOString();
