 ...
 * it via `gateRowCount` and hands it to the caller's own `hash`/`envelope`/`manifestRecordCount`/
 ...
 *
 * SCOPE, deliberately not supported (season-ingest-roster.md §1.2/§2.1) — `roster` is the one
 * season-keyed family this does NOT convert, for exactly these two reasons:
 *   - side-effecting branch hooks. `messages.dedup`, `messages.afterWrite` and
 *     `messages.afterManifest` return a string to log; none of them may perform I/O. `roster`
 *     needs to WRITE nflverse/last-checked-roster.json on the dedup-hit branch and again after
 *     the manifest update — two side effects with no string-returning slot to carry them. Every
 *     callback on this helper's surface is data or a pure function; that is what let five
 *     families convert with zero helper changes after slice 2, and an impure hook would be the
 *     first exception.
 *   - reordering dedup → dry-run → force. That sequence (steps 4-6 below) is fixed. `roster`
 *     runs its force gate BEFORE the dry-run exit — the opposite order — so `--dry-run` on a
 *     changed past season throws there instead of reporting a plan. A `forceGateBeforeDryRun`
 *     flag could express it, but it would encode one family's ordering as a branch in the
 *     shared spine rather than a dimension families vary along.
 * Do not add either without reading season-ingest-roster.md §2.1 (the case against) and §6
 * (what adding them properly would require) first.
 *
 ...
 *   Already-derived rows for `season`, or null/empty (gateRowCount(derived) === 0) when not yet
 ...
 * @param {(derived: *) => number} opts.gateRowCount
...
export async function runSeasonKeyedIngest({
  ...
  for (const season of seasons) {
    ...
    // `rc` (from `gateRowCount`) is the PRE-VALIDATE gate count ONLY — it feeds the sparsity
    // gate below and nothing else. `manifestRecordCount(derived)` and `envelope(season,
    // derived)` are called separately, after `validate` (season-ingest-gamelogs.md §1.2/§2.3),
    // and MUST recompute from `derived` rather than reuse `rc`: oline's `validateOline`
    // mutates `teams` in place (rc is pre-drop, the manifest must carry the post-drop count),
    // and gamelogs' `rc` is `parsedRowCount` (pre-crosswalk) while its envelope `rowCount` is
    // the post-crosswalk total from `players` — a different number by design. Reusing `rc` for
    // the manifest would corrupt both families' recordCount silently and permanently, since
    // both are append-only. `gateRowCount` (season-ingest-advstats.md §2.1) names this for
    // exactly that reason — a callback called `rowCount` invited passing the wrong number,
    // since an envelope field is also called `rowCount` for every family. This is deliberately
    // a comment, not a guard — for schedule and teamcontext the two counts are legitimately
    // equal, so an assertion that they differ would be wrong for those two.
    const rc = derived === null ? 0 : gateRowCount(derived);
