...
Other shortcuts: `update:nfl`, `update:cfbd`, `update:ktc`, `import:snapshot`, `enrich`, `grade`, `backtest`, `backtest:inseason`, `panel`, `panel:flip`, `panel:fit`, `validate:enrichment`.
...
| `.github/workflows/` | Ten uniform ingest jobs (weekly, except `nflverse-snaps.yml`, which is yearly) delegate to the reusable `_ingest.yml` (`workflow_call`) template. Six are deliberately **not** callers: `nflverse-playerstats.yml`, `weekly-ktc.yml`, `cron-deadman.yml`, `smoke-test.yml`, `registry-mirror.yml`, `daily-snapshot.yml` (D1b — runs a browser against the app repo's own build, a shape `_ingest.yml` cannot express; **phase 2** — `cron: "29 16 * * *"` is live since CR-22 landed in both registries, in addition to the `workflow_dispatch` trigger). Triggers, per-job detail and why each standalone cannot delegate: README → [GitHub Actions](README.md#github-actions). Purge URLs: Invariant 8 |
