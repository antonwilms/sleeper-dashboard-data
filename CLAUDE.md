...
| `lib/cfbd.mjs` | CFBD API fetch helpers; `pivotCfbdRows` (E2 Phase B) — the long-form→pivoted-envelope transform, shared by `scripts/update-cfbd.mjs` and `scripts/migrate-college-pivot.mjs` (one implementation, ported from the app's `pivotStatRows`); emits `players` in ascending-numeric-`playerId` order for deterministic bytes |
| `lib/enrichment.mjs` | Enrichment schema validation helpers |
| `lib/io.mjs` | File I/O utilities (`readJson`, `writeJsonStable`, `setStepOutput`); `stableHash` + `sortObjectKeys` (shallow) / `deepSortKeys` (recursive — E2 Phase B, for the CFBD pivot's nested playerId/statType key-order insensitivity) normalisers |
...
| `scripts/migrate-college-pivot.mjs` | E2 Phase B one-shot rewrite of all 27 `college/{passing,receiving,rushing}/<year>.json` files from a long-form row array to the pivoted envelope (`lib/cfbd.mjs` `pivotCfbdRows`), bumps manifest to schemaVersion 2 (`recordCount` = player count), idempotent (skips an already-pivoted file); pure local transform, no network (script-produced, Invariant 1 exception; `--dry-run` supported). See also `scripts/verify-college-pivot-roundtrip.mjs`, the one-shot §4 step 3 proof script (not a committed test — its output is committed instead, at `verification/college-pivot-phase-b-roundtrip-proof.txt`) |
| `scripts/update-cfbd.mjs` | CFBD college stats update logic — fetches long-form rows, pivots via `lib/cfbd.mjs` `pivotCfbdRows`, validates and writes the pivoted envelope (E2 Phase B); dedup hashes the pivoted form on both sides (`cfbdHash` with `deepSortKeys`, since a fresh fetch's row order and each record's statType-key order are not otherwise deterministic) |
...
| `lib/nflverse.mjs` | nflverse fetch + CSV-parse helpers (roster / draft / playerids / advstats / gamelogs / teamcontext / oline); exports `MIN_ROSTER_IDS`, `MIN_DRAFT_YEAR`, `MIN_PLAYERID_ROWS`, `MIN_ADVSTATS_ROWS`, `AY_PER_TARGET_MIN`, `AY_PER_TARGET_MAX` (air-yards-per-target plausibility band shared by `validateAdvStats` and `validateGameLogs`, a validation band not a coverage floor), `MIN_SCHEDULE_SEASON`, `MIN_SCHEDULE_GAMES`, `parsePlayerGameLogs`, `rekeyGameLogsBySleeper`, `MIN_PLAYERGAME_ROWS`, `MIN_GAMELOG_SEASON`, `MIN_TEAMCONTEXT_ROWS`, `MIN_TEAMCONTEXT_SEASON`, `fetchPbpCsv`, `aggregateTeamContext`, `eraTeam`, `MIN_OLINE_ROWS`, `MIN_OLINE_SEASON`, `fetchDepthChartsCsv`, `aggregateOlineStates`, `isoWeekKey`, `OLINE_SLOTS` sparsity constants + pbp-derivation exports |
