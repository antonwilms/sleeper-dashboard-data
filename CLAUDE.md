...
| `.github/workflows/` | Eight uniform weekly ingest jobs delegate to the reusable `_ingest.yml` (`workflow_call`) template. Five are deliberately **not** callers: `nflverse-playerstats.yml`, `weekly-ktc.yml`, `cron-deadman.yml`, `smoke-test.yml`, `daily-snapshot.yml` (D1b — runs a browser against the app repo's own build, a shape `_ingest.yml` cannot express; `workflow_dispatch` only in phase 1, no `cron:` line yet, see CR-22). Triggers, per-job detail and why each standalone cannot delegate: README → [GitHub Actions](README.md#github-actions). Purge URLs: Invariant 8 |
