...
- **Direction:** data→app
- **Triggers:** `OFF_SUM_FIELDS`, `sumRegOff`, `sumRegDef` in `src/utils/environment.js`, `useTeamHistoryLoader` in `src/hooks/useTeamHistoryLoader.js`, `loadTeamSeasonSummary` in `src/api/teamSeasonSummary.js`, `isValidTeamSeasonSummary` in `src/api/dataStore.js`  ‖  `scripts/build-team-season-summary.mjs`, `summariseTeamSeasons` in `lib/teamSummary.mjs`, `validateTeamSeasonSummary` in `lib/validate.mjs`, `nflverse/team-season-summary.json`
