...
- **Data side:** `snapshots/<date>.json`, `bin/update.mjs snapshots`, `scripts/register-snapshots.mjs`, `scripts/grade-snapshot.mjs` (`deriveTargetSeason:34` is the v1-only fallback; envelope reads at `:177`/`:241`/`:326`), `lib/grade.mjs` (scores the snapshot payload), `scripts/panel-run.mjs` `resolveScoring` (`:70-77`, reads `snapshot.scoringSettings`), `bin/import-snapshot.mjs`, README snapshot section
...
- **Data side:** `nfl/season-totals/<year>.json` (written v4 — F-24, 2026-08-24: `idp_*`/`punt*` pruned from every non-`TEAM_*` row's `stats`, minified), `lib/sleeper.mjs` `aggregateWeeks:231` (dominant-team derivation) and `normalizeTeamForSchedule` at `:342` (writes the per-season `team`), `scripts/update-nfl.mjs` (the writer, `:93`; aggregate call `:54`, validate call `:58`), `scripts/migrate-f24-prune.mjs` (the F-24 schemaVersion 3→4 rewrite writer — produced the state CR-02's own `Invariant` describes), `lib/validate.mjs` `validateNflSeason:100`, `lib/backtest.mjs` `isTeamAggregateId` (the data-side `TEAM_` filter), `lib/panel.mjs` `buildTeamTotalsForSeason` (`:75`, applies that filter), `data-catalog.md` season-totals row
...
- **Direction:** data→app
- **Triggers:** `getManifestEntry` and the validator block in `src/api/dataStore.js`, the direct `manifest.files` / `lastModified` reads in `src/utils/ktcHistory.js` (`:92-126`)  ‖  `updateManifestEntry` / `readManifest` / `setManifestInProgress` in `lib/manifest.mjs`, `manifest.json`
...
- **Data side:** `nflverse/advstats/<year>.json`, `bin/update.mjs advstats`, `scripts/update-advstats.mjs`, `lib/nflverse.mjs` `aggregateAdvReceiving` (the producer — computes `targetShare`/`airYardsShare`/`wopr`/`racr`/`components`, the exact served shape this entry's `Invariant` pins), `MIN_ADVSTATS_ROWS:35` (**the definition**) and `AY_PER_TARGET_MIN`/`AY_PER_TARGET_MAX` (the ingest-blocking air-yards plausibility band, advstats-2016-gate.md), `lib/validate.mjs` `validateAdvStats:407`
...
- **Data side:** `nflverse/gamelogs/<year>.json`, `bin/update.mjs gamelogs`, `scripts/update-gamelogs.mjs`, `lib/nflverse.mjs` `MIN_PLAYERGAME_ROWS:48` (**the definition**) + `parsePlayerGameLogs` / `rekeyGameLogsBySleeper`, `lib/validate.mjs` `validateGameLogs:503`
...
- **Direction:** both
- **Triggers:** `src/api/nflGameLogs.js`, `isValidGameLogs` + `MIN_PLAYERGAME_ROWS` in `src/api/dataStore.js`, `resolvePlayerTeam` in `src/utils/playerTeam.js`, and (dp-v2 Slice 5b) `src/utils/seasonEfficiency.js`  ‖  `scripts/update-gamelogs.mjs`, `MIN_PLAYERGAME_ROWS` / `parsePlayerGameLogs` / `rekeyGameLogsBySleeper` in `lib/nflverse.mjs`, `validateGameLogs` in `lib/validate.mjs`, fetchPlayerStatsCsv / STATS_BASE in lib/nflverse.mjs (:412 / :69 — the shared stats_player release path; the STATS_BASE tag-switch is what closed the 2019 gap on 2026-07-03)
...
- **Direction:** data→app
- **Triggers:** `passerRating` in `src/utils/efficiencyMetrics.js`, the `compPct` line in `src/utils/nflStats.js`  ‖  the `Object.entries(stats)` sum loop in `lib/sleeper.mjs` `aggregateWeeks:297`, the writer `scripts/update-nfl.mjs:93`, `validateNflSeason` in `lib/validate.mjs`, `RATE_KEYS` in `lib/fantasyPoints.mjs:29` (read-side; unrelated to F-24's write-side `idp_*`/`punt*` denylist)
...
- **Data side:** `lib/fantasyPoints.mjs` — a hand-maintained mirror (`calculateFantasyPoints:9`, plus `RATE_KEYS:29`); imported by `scripts/grade-snapshot.mjs:20`, which defines `buildInBasisOutcomes:87` (applying the port at `:90`/`:109`); that builder is consumed by in-basis grading (`scripts/grade-snapshot.mjs:140`/`:441`) **and** by `scripts/panel-run.mjs:92` on the R3-FIT path
...
- **Data side:** `nfl/season-totals/<year>.json`, written by the generic sum-all-keys loop in
  ...
  `lib/fantasyPoints.mjs:29` (the sole *read-side* key filter — none of these five is in it). As of
...
- **Triggers:** `market/Market.jsx`'s Efficiency-set call sites, `utils/usageEfficiency.js`'s
  ...
  in `lib/validate.mjs`, `RATE_KEYS` in `lib/fantasyPoints.mjs:29`, `prunePlayerStats` in
