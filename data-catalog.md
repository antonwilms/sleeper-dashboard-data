...
- **schemaVersion:** 1
- **Sparsity gate:** `MIN_SNAPS_ROWS = 3000` (skill-position REG rows, zero-snap appearances included — measured live 2026-09-06: 2016 = 6,464 rows, 2022 = 7,033 rows; ~50% floor, mirroring `MIN_PLAYERGAME_ROWS`' own rationale on a population of the same order of magnitude). **This is NOT the sparsity threshold `runSeasonKeyedIngest` skips-and-continues on** — `scripts/update-snaps.mjs` deliberately passes the spine a near-inert floor (1), because every season this family backfills is already complete/published, so a shortfall here is a truncated fetch, not an unpublished season; `lib/validate.mjs` `validateSnaps` enforces the real `MIN_SNAPS_ROWS` floor as a hard throw instead. Join-rate floor `SNAPS_JOIN_RATE_MIN = 0.85` over skill-position players — measured live 2026-09-06 at 0.981–0.999 across the 2013–2025 backfill (a floor against a broken crosswalk, not a target)
