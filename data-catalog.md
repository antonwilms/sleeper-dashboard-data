...
registry ([cross-repo-registry.md](cross-repo-registry.md)) says otherwise — it never silently feeds projection/scoring/grading. Gaps are honest: an
...
- **`team` (v3+, per-season):** scoring-load-bearing in the app since the R2 flip (2026-07-11) — projection attribution consumes it; the `aggregateWeeks` dominant-team rule is a silent-scoring-change surface (see cross-repo-registry.md, CR-02). F-24's in-place field delete never moves `team` (verified empty diff across all 14 seasons at migration time); D-1's bye inference reads `team` but never writes it.
...
- **schemaVersion:** 1
- **Sparsity gate:** per-row validation (finite int 0–9999, non-empty name, known-or-null position, count 250–600) + Spearman ordering guard (ρ < 0.90) and sentinel-QB landscape check (< 3 in top 15) → `ktc/quarantine/`, unregistered, app-ignored
...
- **Source + provenance:** nflverse **`stats_player`** release (current tag), `stats_player_week_<year>.csv` (weekly rows aggregated season-level; ratios recomputed from components, never summed weekly). **All 14 seasons (2012–2025) now come from the current tag as of the 2026-08-31 re-ingest** — the provenance split that used to exist here (legacy `player_stats` tag for most seasons, current tag for 2019/2025/2016) is retired; every season was re-fetched from the current tag as a side effect of the grain fix below. Historical note, kept for the audit trail: 2019's legacy-tag asset was a permanently broken "starter" upload and 2025 was never mirrored to the legacy tag at all (both filled from the current tag by B1, 2026-07-03); 2016's legacy-tag asset was corrupted (Σ airYards ÷ Σ targets read 3.96 against a 7.80–9.07 archive range) and was re-ingested from the current tag on 2026-08-30, reading 8.48 (advstats-2016-gate.md / reingest-2016.md). Both eras are header-compatible (2025 adds one additive `game_id` column; parsers resolve columns by name).
- **Grain:** player-season, **regular season only** (WR/TE/RB). `aggregateAdvReceiving` filters to `season_type === 'REG'` at ingest (advstats-grain-and-share.md, 2026-08-31) — postseason rows are excluded from both the player totals and the team-share denominators, so a player whose entire season in this file was playoff games is not emitted at all. Postseason is deliberately not served as a `components` sibling block (no consumer exists on either side) but remains one `season_type` filter away in the same source CSV for anyone who later needs it.
...
- **schemaVersion:** 1 (unchanged by the 2026-08-31 grain/weight re-ingest — layout is byte-identical, only values and row membership moved)
- **Sparsity gate:** `MIN_ADVSTATS_ROWS = 250` (cross-repo). `lib/validate.mjs` `validateAdvStats` also asserts a season-level air-yards plausibility band, Σ`airYards` ÷ Σ`targets` ∈ `[AY_PER_TARGET_MIN, AY_PER_TARGET_MAX]` = `[6, 11]` (`lib/nflverse.mjs`, **regular season only** as of 2026-08-31) — a forward guard against the legacy-tag corruption above, not a coverage floor.
...
- **Served path / subcommand / refresh:** `nflverse/gamelogs/<year>.json`; `bin/update.mjs gamelogs [--year|--all]`; Saturday Action (`nflverse-playerstats.yml`) — as of playerstats-single-fetch.md (2026-08-31), this family and advstats are derived from a **single fetch** of `stats_player_week_<year>.csv` by the `playerstats` orchestrator (`bin/update.mjs playerstats`), so the two families can no longer diverge across a day boundary within the same week; the standalone `gamelogs` subcommand above still exists (manual runs/`--all` backfill/smoke test) and fetches independently when invoked directly — `--all` is unaffected by the orchestrator and always fetches per season
...
- **schemaVersion:** 1
- **Sparsity gate:** `MIN_PLAYERGAME_ROWS = 3000` (cross-repo). `lib/validate.mjs` `validateGameLogs` also asserts the same season-level air-yards plausibility band as advstats, Σ`receivingAirYards` ÷ Σ`targets` (all rows, REG+POST) ∈ `[AY_PER_TARGET_MIN, AY_PER_TARGET_MAX]` = `[6, 11]` (`lib/nflverse.mjs`) — a forward guard against the legacy-tag corruption above, not a coverage floor. **As of the 2026-08-31 advstats grain fix, this basis is no longer shared with advstats** — gamelogs stays REG+POST (unfiltered `parsePlayerGameLogs`), while advstats became regular-season-only; the two families now use the *same* `AY_PER_TARGET_MIN`/`MAX` constants against *different* underlying row sets, not a shared basis (advstats-grain-and-share.md §3.4). Deliberately does NOT also carry advstats' `|airYardsShare| ≤ 1` bound: five per-game rows legitimately exceed it (team net air yards near zero on behind-the-line passes; gamelogs-airyards-gate.md §1.3).
...
- **schemaVersion:** 1
- **Sparsity gate:** `MIN_OLINE_ROWS = 160` (internal — see the non-entry note in cross-repo-registry.md; no app counterpart)
...
## nflverse snap counts (PFR-sourced offensive snap shares)
- **Served path / subcommand / refresh:** `nflverse/snaps/<year>.json`; `bin/update.mjs snaps [--year|--all]`; yearly Action (`nflverse-snaps.yml`, mid-February — post-season, in-season weekly refresh out of scope for this family)
- **Source + provenance:** nflverse `snap_counts` release (PFR-sourced), `snap_counts_<year>.csv`. **R1-SNAPS (this family) exists to widen the panel/backtest's snap-share predictor past Sleeper's own `off_snp`/`tm_off_snp`, which are only populated 2020+** — see `nfl/season-totals`'s coverage row
- **Grain:** player-season, aggregated from player-game rows; REG only (same rule as advstats/gamelogs)
- **Join id(s):** `pfr_player_id` → `sleeper_id` via the D2 crosswalk (`nflverse/playerids.json` `.bySleeper[*].pfrId`, reversed in-memory — there is no natural pfrId-keyed index on disk)
- **Coverage:** 2013–2025 (`MIN_SNAPS_SEASON = 2013`, **not** the 2012 floor the other four nflverse families share — measured live 2026-09-06: `snap_counts_2012.csv` exists upstream [HTTP 200] but is header-only with zero data rows; 2013 is the first season with real rows)
- **schemaVersion:** 1
- **Sparsity gate:** `MIN_SNAPS_ROWS = 3000`, enforced **post-crosswalk** — `validateSnaps` sums `games` over the joined `players` map only, not the pre-join `byPfrId` population. `aggregateSnapCounts`'s own `rowCount` (the number this constant's rationale was originally measured against — 2016 = 6,464 rows, 2022 = 7,033 rows, pre-crosswalk) is close but not identical to what the floor actually gates: post-crosswalk, 2016 = 6,397 rows and 2022 = 7,024 rows. 3000 is still comfortably the intended ~50% floor of the post-crosswalk counts (mirroring `MIN_PLAYERGAME_ROWS`' own rationale on a population of the same order of magnitude), so the constant needs no change — only this note did. **This is NOT the sparsity threshold `runSeasonKeyedIngest` skips-and-continues on** — `scripts/update-snaps.mjs` deliberately passes the spine a near-inert floor (1), because every season this family backfills is already complete/published, so a shortfall here is a truncated fetch, not an unpublished season; `lib/validate.mjs` `validateSnaps` enforces the real `MIN_SNAPS_ROWS` floor as a hard throw instead. Join-rate floor `SNAPS_JOIN_RATE_MIN = 0.85` over skill-position players — recomputed from the thirteen committed 2013–2025 files, **0.854–0.995**, not the originally-quoted "0.981–0.999" (fix pass 1 item 1). The floor season is **2013 at 0.8537**, clearing `SNAPS_JOIN_RATE_MIN` by only 0.0037 (about two players): the D2 `pfrId` crosswalk is bounded by Sleeper's own player universe, and a 2013 player who retired before Sleeper existed has no id and never will — attrition, not a defect, and the same ~0.853 effect independently measured in D5's depth-chart planning. The margin at 2013 is thin enough that a crosswalk regression would trip it
- **Team-snap denominator (an assumption, not a certainty):** a team's offensive snaps for a game is the **MAX** `offense_snaps` across every player who appears for that team in that game (computed over ALL positions, including OL — not just the emitted skill positions). Holds because an offensive lineman who never leaves the field makes "max" a good proxy for the team's true offensive play count; measured team-game medians 66–67 with a 41–100 range are consistent with real offensive play counts. Team codes pass through **unchanged** — this source is already historically team-coded per season (2016 carries `LA`/`SD`/`OAK`, matching each franchise's real 2016 name) — `eraTeam()` must never be applied here; `validateSnaps` asserts the three era boundaries (`STL`↔`LA` 2015/2016, `SD`↔`LAC` 2016/2017, `OAK`↔`LV` 2019/2020)
- **Null semantics:** `offPct` is Σ `offSnaps` ÷ Σ (per-team-stint, week-restricted) `teamOffSnaps` — a straight ratio of two honestly-summed components, never an average of a stored per-game rate (CR-10); `null` when a needed team denominator is unusable. Traded players get week-restricted per-team-stint denominators (mirrors `aggregateAdvReceiving`'s traded-player treatment) — a stint's denominator sums only the team-game maxima for the weeks the player actually has a row with that team, never the team's season-wide total
- **`byPfr` bucket — a deliberate departure from the advstats/gamelogs house pattern:** those two families drop unmapped rows to a bare `unmapped` count; at this family's ~99% join rate the unmapped residue is small and worth retaining (the rows that fail to join are the interesting ones), so they are kept in `byPfr`, keyed by their original `pfr_player_id`
- **Served shape:** `{ schemaVersion: 1, season, generatedAt, rowCount, playerCount, unmapped, players, byPfr }` — `players` keyed by `sleeper_id` → `{ pfrId, name, position, team, [traded, teams,] games, offSnaps, teamOffSnaps, offPct }`; `byPfr` same shape, keyed by `pfr_player_id`, for unmapped rows only
- **Consumption:** **ENABLED (D6a, 2026-09-06).** `scripts/panel-run.mjs`'s `DEFAULT_LOAD.loadSnapShare` now re-points at `nflverse/snaps/<year>.json`; `PANEL_DEFAULTS.fromYear` flipped 2020→2013 (`lib/panel.mjs`); `resolveSnapCounts` (`lib/panel.mjs`) is the additive fallback consulted by `computeSnapShare`, `buildCohortPools`'s snap pool, and `attachFactorMultipliers`' `snapShare` branch — season-totals' own `off_snp`/`tm_off_snp` win when present, this family fills the pre-2020 gap, never overrides a real 2020+ value. `lib/backtest.mjs:290`/`bin/backtest.mjs:119` (the standalone D3 self-validation tool) were revisited and intentionally left unwidened — that tool has no injectable loader for this family; its own "pre-2020 dropped" comment is still accurate for what it reads. **Cross-validation measured 2026-09-06 over the 2020–2024 overlap** (this family vs. Sleeper's own `off_snp`/`tm_off_snp`, RB/WR/TE — the real `snapShare` cohort scope): n=2,643, Pearson r=0.9995, mean absolute difference=0.0017 (QB/RB/WR/TE panel-position scope: n=3,057, r=0.9993, MAD=0.0019) — cleared the r≥0.95 gate with a wide margin before this enable
- **Keep-rationale:** banks the 2013–2019 snap-share family (plus the 2020–2024 overlap needed to validate it) against Sleeper's `off_snp`/`tm_off_snp`, which only exist from 2020 — the effective grading panel is 2020–2024 today solely because of this gap; this ingest is the prerequisite for eventually widening it

## nflverse historical depth charts (D5)
- **Served path / subcommand / refresh:** `nflverse/depth/<year>.json`; `bin/update.mjs depth [--year|--all]`; Saturday Action (`nflverse-depth.yml`, weekly in-season — not yearly like snaps, since current-season QB1-change capture is the point)
- **Source + provenance:** nflverse `depth_charts` release, `depth_charts_<year>.csv`. **Two schemas share one release, split at `DEPTH_ESPN_FROM_SEASON = 2025`**: legacy (2013–2024, `season,club_code,week,game_type,depth_team,…,depth_position,…`, `aggregateDepthLegacy`) and ESPN (2025+, the same `dt` schema `aggregateOlineStates` reads, `aggregateDepthEspn`). ESPN-era NFL-week derivation reads the **upstream** nfldata `games.csv` directly (`fetchSchedulesCsv` → `buildGamedayIndex`), never the served `nflverse/schedule` family, which carries no date column at all — CR-08 is not touched
- **Grain:** team × NFL-week × position, ordered array = depth order (with one exposed exception — see Null semantics). ESPN rows are reduced from a near-daily snapshot cadence to one chart per (team, NFL week) by keeping only the rows at that bucket's max `dt` (shares `addToLatestDtBucket` with `aggregateOlineStates`); the winning `dt`/`date` is retained per (week, team)
- **Join id(s):** `gsis_id` → `sleeper_id` via the crosswalk's `.ids` map (primary; 100% populated on legacy, 98.6% on ESPN skill rows); ESPN rows whose `gsis_id` is empty fall back to `espn_id` → `sleeper_id` via `crosswalkFromBySleeper(bySleeper, 'espnId')` — generalizes D4's `pfrCrosswalkFromBySleeper` inversion pattern rather than duplicating it. **Prerequisite:** an ESPN-era season hard-errors if the crosswalk's `bySleeper` is absent or empty (finding 3b) — regenerate to schemaVersion 2 first (`node bin/update.mjs playerids`)
- **Coverage:** 2013–present (`MIN_DEPTH_SEASON = 2013`) — a floor chosen for join/gate defensibility, not upstream absence (the `depth_charts` release itself goes back to 2001); a below-floor `--year` is rejected in `scripts/update-depth.mjs` before the spine ever sees it, since (unlike snaps' 2012) pre-2013 assets carry real rows and would not naturally route through the "not published" skip
- **schemaVersion:** 1
- **Sparsity gates (all in `validateDepth`, not the spine — see Null semantics/Risks):** `MIN_DEPTH_ROWS = 3,500` over joined (non-null) skill rows — measured 2026-09-06 post-join: 2013 (floor, legacy) = 7,718; 2024 (legacy) = 8,611; 2025 (ESPN, post-reduction) = 10,411; 30–32 teams present at week 1 (measured exactly 32 in every season **except 2017**, which has 30 — the MIA@TB week-1 game was postponed to week 11 that season for Hurricane Irma, so both teams' legacy depth-chart rows are genuinely absent from week 1 upstream, not a parser fault; the gate mirrors `MIN_DEPTH_QB1_TEAMS`'s own slack rather than special-casing one season); a depth-1 QB present for ≥30 teams at week 1 (`MIN_DEPTH_QB1_TEAMS = 30`, measured 31/2013, 32/2016, 32/2017, 32/2024 — a gate of 32 would fail the floor season on correct data); skill-position join rate ≥ `DEPTH_JOIN_RATE_MIN = 0.80` over **distinct** ids (not row occurrences — see Null semantics), measured 0.8529/2013 (the floor), 0.9948/2024, 0.9760/2025; per-year team-code assertions at the three boundary pairs (`STL`↔`LA` 2015/2016, `SD`↔`LAC` 2016/2017, `OAK`↔`LV` 2019/2020) — this source is already historically team-coded like snaps; `eraTeam()` must never be applied to it. `scripts/update-depth.mjs` passes the spine a near-inert `minRows: 1` (mirrors `scripts/update-snaps.mjs`'s worked pattern exactly) so a legacy filter mistake can never silently skip a backfill season green
- **Null semantics — two departures from the family's own house pattern, both deliberate:**
  - **An unresolved join is kept as `null` at its original array index, never dropped.** Dropping would shift every later entry down a slot, silently promoting a depth-2 player into the depth-1 slot whenever the true depth-1 player fails to join — verified against finding 5's own measured QB1 number: with position preserved, 2013 measures exactly 31 teams with a joined depth-1 QB; a drop-and-reindex implementation measures 32 (it would credit a team whose *backup* QB happens to have joined), silently contradicting the number `MIN_DEPTH_QB1_TEAMS` is calibrated against. This is the one respect in which the served array is `sleeper_id|null`, not the plain `sleeper_id` its sibling families use.
  - **Legacy `depth_team` is not a total order within a position (finding 6).** Measured on 2013: 2,125 of 6,342 `(club, week, position)` groups share a `depth_team` value, because legacy ranks **within `depth_position`** (the slot) while ESPN's `pos_rank` ranks **within `pos_abb`** (the position) — the two eras measure different things, not the same rank on different schemas. The served array is still deterministic (`depth_team asc, depth_position asc, input order`), but a per-team `depthPositions` sibling — parallel-indexed (nulls included) to each position's id array — is emitted **only when at least one entry carries a non-null `depthPosition`**, i.e. only on legacy-era files; ESPN-era files never have it. The field's mere presence, not just its content, is the cross-era signal — a consumer that only reads the plain arrays would otherwise read the 2024→2025 boundary as a real change in the world rather than a change in what's being measured.
  - Otherwise standard: `unmapped`/`outOfWindow` are honest counts, never fabricated; `week1Qb1[team]` is `null` for a team with no resolved depth-1 QB; `qb1Changed` is `null` at `MIN_DEPTH_SEASON` (no prior season exists) and **throws** (does not silently yield `null`) if a later season's prior-year served file is missing — a silent `null` there would be indistinguishable from the floor season's legitimate one
- **Served shape:** `{ schemaVersion: 1, season, generatedAt, rowCount, playerCount, unmapped, outOfWindow, weeks, week1Qb1, qb1Changed }` — `rowCount`/`playerCount` count resolved (non-null) entries only; `weeks: { [week]: { [team]: { QB: [sleeperId|null,…], RB: […], WR: […], TE: […], [depthPositions], [dt, date] } } }`; `week1Qb1: { [team]: sleeperId|null }` is the depth-1 QB at week 1 REG (index 0, not "any QB present"); `qb1Changed: { [team]: boolean } | null`. `dt`/`date` are present only on ESPN-era team entries (the winning snapshot moment for that team-week's reduction)
- **A later correction to season Y−1 can never propagate into Y**, because Y is completed and `--force`-gated under Invariant 1, so `qb1Changed` for Y goes stale with nothing to detect it — the same limitation `qb1Changed`'s own design accepts by reading a prior *served file* rather than recomputing. A relocation year (e.g. 2016's `STL`→`LA`) also reads as `qb1Changed: true` for the new team code even when the same QB started both seasons, since the comparison is keyed by team code with no era-remap — a known, documented limitation, not a bug
- **Consumption:** **Wired (D6a, 2026-09-06).** `lib/panel.mjs`'s `attachFactorMultipliers` reads `weeks`' latest-available-week depth order for the `depth` factor branch (Step 8 reconstruction) and for QB1 identity (Step 7b `qbQuality`'s `resolveQb1`) — capture-and-measure only, no app/served-family change. Still no app path reads this family
...
- **`raw/`** — legacy one-time dumps (league/roster/user/player-state Sleeper snapshots, 6 files). These 6 are registered in `manifest.json` (the only entries in this section that are) but are not a served family. The earlier 252 `raw/stats-*.json` dumps referenced by `.claude/tasks/retire-raw-stats.md` have already been retired and removed from disk. The eight `raw/cfbd-players-<year>.json` dumps (2017–2024) have likewise been retired and removed (`.claude/tasks/repo-weight.md`, E1) — nothing read them, and they remain recoverable from commit `82235d2`.
- **`ktc/quarantine/`** — script-produced when a KTC scrape trips the Spearman ordering guard or the sentinel-QB landscape check; unregistered, app-ignored; does not exist yet — created on demand by `scripts/update-ktc.mjs` on the first guard trip (no scrape has been quarantined).
