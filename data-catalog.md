...
- **Consumption:** **capture-only, banked disabled.** `scripts/panel-run.mjs`'s `DEFAULT_LOAD.loadSnapShare` seam (built for exactly this slice) stays `null` — the four sites that would need to read this family together (`lib/panel.mjs:181,915-917,1135-1136`, `lib/backtest.mjs:291-292`) are **unmodified** in this slice. **Cross-validation measured 2026-09-06 over the 2020–2024 overlap** (this family vs. Sleeper's own `off_snp`/`tm_off_snp`, RB/WR/TE — the real `snapShare` cohort scope): n=2,643, Pearson r=0.9995, mean absolute difference=0.0017 (QB/RB/WR/TE panel-position scope: n=3,057, r=0.9993, MAD=0.0019) — clears the r≥0.95 gate with a wide margin, so the two sources measure essentially the same thing. **The panel stays 2020+ (`PANEL_DEFAULTS.fromYear`) regardless** — enabling the fallback is a separate three-edit change (re-point `loadSnapShare`, flip `fromYear`, revisit the pre-2020-undroppable backtest assertions, and re-fit CR-15's factor multipliers before further exponent activation) that was deliberately deferred past this slice, not blocked by the measurement
...
- **Consumption:** **capture-only — wires no factor.** No app, projection, grading, backtest, or panel path reads this family. The panel work that consumes `weeks`' depth order and `qb1Changed` belongs to a later slice (D6)
